 ...
 */
export function detectVisualizer(vars: VarMap, currentEvent?: any, pointerContext: Record<string, string[]> = {}): CanvasState[] {
  ...
  for (const name of keys) {
    ...
    if (
      ...
      const graphState = tryDetectAdjacencyGraph(
        name, val, type, currentEvent, vars, keys, consumedKeys, pointerContext
      );
      if (graphState) {
