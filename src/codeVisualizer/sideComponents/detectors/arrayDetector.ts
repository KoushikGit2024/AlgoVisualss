import {type CanvasState,type VarMap, deepUnwrap, isFlatArray, is2DArray, extractEventIndices } from './coreUtils';
  ...
  keys.forEach(key => {
    ...
    if (typeof val === 'string') {
      const { pointers, usedKeys: ptrKeys } = collectIndexPointers(
        keys, vars, key, pointerContext, ['i', 'j', 'k', 'left', 'right', 'mid', 'curr', 'ptr']
      );
    ...
    if (is2DArray(val)) {
      ...
      for (const rKey of potentialRowKeys) {
        for (const cKey of potentialColKeys) {
          ...
          const rBoundElsewhere = pointerContext[rKey] && !rContext && pointerContext[rKey].length > 0;
          const cBoundElsewhere = pointerContext[cKey] && !cContext && pointerContext[cKey].length > 0;
    ...
    if (Array.isArray(val) || val.__type === 'container') {
      ...
      if (typeof window !== 'undefined' && window.location.href.includes('/algorithms/sorting') && key === 'arr') {
        arrayType = 'sortbars';
      }

      visualizers.push({
        id: key, type: arrayType, usedKeys,
        props: { value: val, pointers, readIndices: reads, writeIndices: writes },
      });
      return;
    }
