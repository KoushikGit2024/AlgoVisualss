...
export function collectGraphPointers(
  keys: string[], vars: VarMap, targetName: string, pointerContext: Record<string, string[]>
  ...
  for (const key of keys) {
    ...
    const isExplicitlyBound = lower.startsWith(`${targetName.toLowerCase()}_`) || lower.endsWith(`_${targetName.toLowerCase()}`);
    
    // Contextual binding (via history tracker)
    const isContextuallyBound = pointerContext[key]?.includes(targetName);

    const isMatch = isExplicitlyBound || isContextuallyBound || pointerPatterns.some(
      p => lower === p || lower.startsWith(`ptr_${p}`) || lower.startsWith(`${p}_`) || lower.endsWith(`_${p}`)
    );

    // If it's contextually bound to another structure, DO NOT broadcast it here.
    const isBoundElsewhere = pointerContext[key] && !pointerContext[key].includes(targetName) && pointerContext[key].length > 0;
...
export function collectIndexPointers(
  keys: string[], vars: VarMap, targetName: string, pointerContext: Record<string, string[]>, patterns: string[] = []
  ...
  for (const key of keys) {
    ...
    const isMatch = isExplicitlyBound || isContextuallyBound || patterns.some(
      p => lower === p || lower.startsWith(`ptr_${p}`) || lower.startsWith(`${p}_`) || lower.endsWith(`_${p}`)
    );
...
export function collectNodePointers(
  keys: string[], vars: VarMap, targetName: string, pointerContext: Record<string, string[]>, patterns: string[], nodeArray?: any[]
): { pointers: { name: string; nodeId: string }[]; usedKeys: string[] } {
  const pointers: { name: string; nodeId: string }[] = [];
  const usedKeys: string[] = [];
  ...
  for (const key of keys) {
    ...
    const isNameMatch = isExplicitlyBound || isContextuallyBound || patterns.length === 0 || patterns.some(
        p => lower === p || lower.endsWith(`_${p}`) || lower.startsWith(`ptr_${p}`)
    );
