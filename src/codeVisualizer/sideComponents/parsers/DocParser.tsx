...
/* ─── Markdown-link-aware cell renderer ────────────────────────────────────── */
/**
 * Table cells (and only table cells) may contain a single markdown-style
 * link produced by the data-generation script, e.g. "[Fenwick Tree (BIT)](/algorithms/range_structures/fenwick)".
 * This parses that pattern into a real anchor; any cell that isn't a full
...
const ClickableTableRow = ({ row }: { row: string[] }) => {
  ...
  // Scan row to find if any cell has a link (to make the whole row clickable)
  ...
  return (
    <tr
      ...
      className={cn(
        "group border-b border-border last:border-b-0 odd:bg-surface even:bg-surface-2/40 transition-colors",
        rowHref ? "cursor-pointer hover:bg-accent/10" : "hover:bg-accent/5",
      )}
    >
      {row.map((cell, j) => (
        <td
          key={j}
          className="px-3 py-2 border-r border-border last:border-r-0 text-muted whitespace-nowrap"
        >
...
const ScrollableTableWrapper = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="relative my-4 rounded-lg border border-border group overflow-hidden">
      {/* Right indicator */}
      <div
        className={cn(
          "absolute right-0 top-0 bottom-0 w-8 pointer-events-none z-10 flex items-center justify-end pr-2 transition-opacity duration-300",
          canScrollRight ? "opacity-100" : "opacity-0",
        )}
      >
        <div className="bg-surface-2 p-1 rounded-full shadow-md border border-border animate-pulse">
        ...
        return (
          <ScrollableTableWrapper key={index}>
            <table className="w-full text-left border-collapse text-[12.5px]">
              ...
              <tbody>
                {node.rows.map((row, i) => (
                  <ClickableTableRow key={i} row={row} />
                ))}
              </tbody>
            </table>
