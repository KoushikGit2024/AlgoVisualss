...
export function usePointerHistory(snapshots: any[]): PointerHistoryMap {
  return useMemo(() => {
    ...
    for (let step = 0; step < snapshots.length; step++) {
      ...
      if (event && (event.type === 'READ' || event.type === 'ASSIGNMENT' || event.type === 'ASSIGN' || event.type === 'DECLARE' || event.type === 'DECLARATION')) {
        ...
        if (typeof varString === 'string') {
          ...
          if ((event.type === 'ASSIGNMENT' || event.type === 'ASSIGN' || event.type === 'DECLARE' || event.type === 'DECLARATION') && !varString.includes('[')) {
