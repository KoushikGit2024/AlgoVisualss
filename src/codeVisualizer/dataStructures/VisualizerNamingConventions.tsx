    ...
    <div className="flex flex-col gap-8 pb-10 text-text">
      ...
      <section className="bg-surface-2 p-5 rounded-xl border border-border">
        ...
        <p className="text-sm text-muted leading-relaxed mb-4">
          The <strong className="text-text">VisualGround</strong> engine's heuristic detector parses
          the AST runtime scope and automatically mounts the correct React component based on
          variable naming prefixes and value shape.
        </p>
        <div className="flex flex-col gap-2">
          <div className="flex items-start gap-2 bg-surface p-3 rounded-lg border border-border">
            <span className="w-5 h-5 rounded-full bg-accent/20 text-accent flex items-center justify-center text-xs font-bold shrink-0">
              1
            </span>
            <p className="text-[calc(13rem/16)] text-muted">
              <strong className="text-text">PREFIX match</strong> — Does the variable name start
              with a known trigger prefix?
            </p>
          </div>
          <div className="flex items-start gap-2 bg-surface p-3 rounded-lg border border-border">
            <span className="w-5 h-5 rounded-full bg-accent/20 text-accent flex items-center justify-center text-xs font-bold shrink-0">
            ...
            <p className="text-[calc(13rem/16)] text-muted">
              <strong className="text-text">SHAPE validation</strong> — Is the runtime value
              actually the correct data shape?
            </p>
          </div>
        </div>
        <p className="text-[calc(12rem/16)] text-muted mt-3 italic">
          Example: `numRows = 5` will not trigger &lt;D1Array /&gt; even though it starts with
          `num`, because `5` is a scalar. Only variables whose value passes both checks get
          visualized.
        </p>
      </section>
          ...
          <table className="w-full text-left border-collapse text-[calc(13rem/16)]">
            <thead>
              <tr className="bg-surface-2">
                <th className="px-4 py-3 border-b border-r border-border font-semibold text-text">
                  Priority
                </th>
                <th className="px-4 py-3 border-b border-r border-border font-semibold text-text">
                  Component
                </th>
                <th className="px-4 py-3 border-b border-border font-semibold text-text">
                  Why first?
                </th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                { p: 4, c: "<D2Array />", w: "2D array shape (matrix)" },
                {
                  p: 5,
                  c: "<LinkedList />",
                  w: "Automatic structure detection (`value` + `next` fields)",
                },
                ...
                <tr
                  key={i}
                  className="border-b border-border last:border-b-0 even:bg-surface-2/30 hover:bg-surface-3 transition-colors"
                >
                  <td className="px-4 py-2.5 border-r border-border text-muted font-mono">
                    {row.p}
                  </td>
                  <td className="px-4 py-2.5 border-r border-border text-accent-3 font-mono">
                    {row.c}
                  </td>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {DATA_STRUCTURES.map((ds, idx) => (
          <section
            key={idx}
            className={cn(`border rounded-xl overflow-hidden ${ds.color.split(" ")[0]} bg-surface`)}
          >
            {/* Header */}
            <div
              className={cn(
                `px-4 py-3 border-b ${ds.color.split(" ")[0]} ${ds.color.split(" ")[1]} flex items-center gap-2`,
              )}
            >
              {ds.icon}
            ...
            <div className="p-4 flex flex-col gap-4">
              ...
              {/* Prefixes */}
              <div>
                <h3 className="text-[calc(11rem/16)] font-bold text-text uppercase tracking-wider mb-2">
                  ...
                  {ds.prefixes.map((pref, i) => (
                    ...
                    >
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 hidden group-hover:block z-50 whitespace-nowrap bg-bg border border-border px-2 py-1 rounded shadow-lg">
                        <code className="text-[calc(10rem/16)] font-mono text-muted">
                          {pref.example}
                        </code>
                      </div>
                    </div>
              ...
              {ds.auxiliary.length > 0 && (
                <div>
                  <h3 className="text-[calc(11rem/16)] font-bold text-text uppercase tracking-wider mb-2 mt-2">
                  ...
                  <div className="overflow-x-auto border border-border rounded-lg styled-scrollbar">
                    <table className="w-full text-left border-collapse text-[12.5px]">
                      <thead>
                        <tr className="bg-surface-2">
                          <th className="px-3 py-2 border-b border-r border-border font-semibold text-text whitespace-nowrap">
                            Role
                          </th>
                          <th className="px-3 py-2 border-b border-r border-border font-semibold text-text whitespace-nowrap">
                            Trigger
                          </th>
                          <th className="px-3 py-2 border-b border-border font-semibold text-text">
                            Notes
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {ds.auxiliary.map((aux, i) => (
                          <tr
                            key={i}
                            className="border-b border-border last:border-b-0 even:bg-surface-2/30"
                          >
                            <td className="px-3 py-2 border-r border-border text-muted whitespace-nowrap">
                              {aux.role}
                            </td>
                            <td className="px-3 py-2 border-r border-border text-accent-3 font-mono">
                              {aux.trigger}
                            </td>
      ...
      <section className="bg-surface-2 p-5 rounded-xl border border-border">
        ...
        <p className="text-[calc(13rem/16)] text-muted mb-4">
          These variable names are <strong className="text-text">never consumed</strong> by the
          detector and will always appear in the Variables panel:
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {[
            ...
            >
              <code className="text-failure font-mono text-[12.5px] bg-failure/10 px-2 py-0.5 rounded shrink-0">
                {item.v}
              </code>
      ...
      <section>
        ...
        <div className="overflow-x-auto border border-border rounded-lg styled-scrollbar">
          <table className="w-full text-left border-collapse text-[calc(12rem/16)]">
            <thead>
              <tr className="bg-surface-2">
                <th className="px-3 py-2.5 border-b border-r border-border font-semibold text-text whitespace-nowrap">
                  Component
                </th>
                <th className="px-3 py-2.5 border-b border-r border-border font-semibold text-text whitespace-nowrap">
                  Primary Prefixes
                ...
                <th className="px-3 py-2.5 border-b border-border font-semibold text-text">
              ...
              {[
                {
                  ...
                  p: "adj, graph, network",
                  s: "2D array",
                  a: "*edge*, *visit*, node/curr/u/v",
                },
                {
                  c: "<TrieTree />",
                  p: "trie, prefix_tree, ptree",
                  s: "Struct OR Array",
                  a: "curr, node, ptr, temp",
                },
                {
                  c: "<Tree />",
                  p: "tree, bst, root, heap, forest",
                  s: "Struct OR Array",
                  a: "root, curr, parent, temp, left, right",
                },
                {
                  c: "<D2Array />",
                  p: "(Any Name)",
                  s: "2D array (Jagged allowed)",
                  a: "r/row/r_*, c/col/c_* (both req)",
                },
                {
                  c: "<D1Array />",
                  p: "(Any Name)",
                  s: "Flat array",
                  a: "i, j, k, left, right, mid, curr, ptr",
                },
                { c: "<SortBars />", p: "arr", s: "Numeric array on sorting pages", a: "i, j, k" },
                {
                  c: "<LinkedList />",
                  p: "(Any Name)",
                  s: "Struct with `value` & `next`",
                  a: "All node pointers automatically attach",
                },
                {
                  c: "<Queue />",
                  p: "queue, deque, line, queue_nodes, tasks",
                  s: "Flat array",
                  a: "front, back, rear, head, tail, curr",
                },
                {
                  c: "<Stack />",
                  p: "stack, stk, history, undo, frames",
                  s: "Flat array",
                  a: "top, peek",
                },
                ...
                >
                  <td className="px-3 py-2 border-r border-border text-text font-mono whitespace-nowrap">
                    {row.c}
                  </td>
                  <td className="px-3 py-2 border-r border-border text-accent font-mono">
                    {row.p}
                  </td>
                  <td className="px-3 py-2 border-r border-border text-muted whitespace-nowrap">
                    {row.s}
      ...
      <section className="bg-surface-2 p-5 rounded-xl border border-border mt-4">
        ...
        <p className="text-[calc(13rem/16)] text-muted leading-relaxed mb-4">
          To prevent browser out-of-memory crashes on enormous algorithms (like backtracking), the
          VisualGround engine features a smart{" "}
          <strong className="text-text">Semantic Hierarchical Compression</strong> system.
        </p>
        <ul className="flex flex-col gap-2 mb-4 text-[calc(13rem/16)] text-muted list-disc list-inside marker:text-ds-string">
          <li>
            <strong>Max Execution Steps:</strong> The engine will halt execution if a single run
            exceeds <code className="text-text">2,000,000</code> operations to prevent infinite
            loops.
          </li>
          <li>
            <strong>Smart Frame Dropping:</strong> If the visualizer accumulates over{" "}
            <code className="text-text">30,000</code> animation frames, it automatically begins
            dropping less-important events to save memory.
          </li>
          <li>
            It first drops <code className="text-text">READ</code> events (which usually make up 90%
            of a loop), keeping all your assignments, function calls, and console logs completely
            intact!
          </li>
          <li>
            If memory pressure continues, it drops <code className="text-text">CONDITION</code> and
            loop events, meaning variables will jump cleanly to their next states without showing
            the intermediate loop overhead.
          </li>
        </ul>
        <p className="text-[calc(13rem/16)] text-muted italic border-l-2 border-accent pl-3 py-1 bg-surface/50 rounded-r">
          <strong>Playback Tip:</strong> The playback speed slider directly sets the exact delay
          between frames in milliseconds (<code className="text-text">1ms - 1000ms</code>) for
          precise, real-time timing control. You can also click the value to type an exact number.
        </p>
      </section>
    </div>
  );
