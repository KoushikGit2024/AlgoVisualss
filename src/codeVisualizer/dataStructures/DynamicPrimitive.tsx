...
export const DynamicPrimitive: React.FC<DynamicPrimitiveProps> = ({ value, depth = 0 }) => {
  ...
  if (Array.isArray(value)) {
    ...
    return (
      <div className="flex flex-row items-center gap-1 border border-border/50 bg-bg/50 rounded-sm p-1">
        {value.map((v, i) => (
          <div key={i} className="px-1.5 py-0.5 min-w-[20px] text-center border border-border/30 bg-surface rounded-sm flex items-center justify-center">
  ...
  if (typeof value === 'object') {
    ...
    if (entries.length === 0) return <span className="text-muted font-mono">{isSet ? 'Set{}' : '{}'}</span>;
