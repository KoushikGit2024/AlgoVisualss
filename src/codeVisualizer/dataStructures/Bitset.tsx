        ...
        <AnimatePresence mode="popLayout">
          {displayBits.map(({ bit, index }) => {
            ...
            const textClass = isActive
              ? "text-cyan-300 text-[calc(15rem/16)]"
              : "text-muted/40 text-[calc(13rem/16)]";
            ...
            return (
              ...
              >
                <motion.div
                  ...
                  className={cn(
                    `w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center font-mono font-bold rounded border transition-all duration-200 ${bgClass} ${borderClass} ${textClass} ${shadowClass}`,
                  )}
                >
                  {bit}
                </motion.div>
