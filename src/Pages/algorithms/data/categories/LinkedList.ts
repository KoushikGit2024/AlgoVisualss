  ...
  about: [
    ...
    { tag: "h2", text: "Singly vs. doubly linked" },
    {
      ...
      rows: [
        ...
        [
          "Traverse backward?",
          "No — requires re-traversal from head",
          "Yes — O(1) per step via prev",
        ],
        [
          "Delete a given node (with reference, no head search)",
          "O(n) — need the previous node",
          "O(1) — prev is already known",
        ],
    ...
    {
      tag: "p",
      text: "A huge fraction of linked-list bugs come from special-casing 'what if the head itself needs to change' (empty list, deleting the head, building a new list from scratch). The standard fix is a dummy/sentinel node placed before the real head — every real node, including what becomes the new head, is then 'the node after some node', eliminating the special case entirely. You'll see this pattern in Merge Sorted Lists and LRU Cache Design below.",
    },
    {
      tag: "note",
      variant: "tip",
      text: "Before writing any linked-list code, always ask: 'will I still have a reference to the rest of the list after this pointer reassignment?' — saving next before overwriting it is the single most common fix for linked-list bugs.",
    },
  ],
  ...
  items: [
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "A Least Recently Used (LRU) Cache is a fixed-capacity key-value store that, when full, evicts the entry that hasn't been accessed for the longest time to make room for a new one. It's one of the most common 'design a data structure' interview questions precisely because the obvious approaches (a plain array, a plain linked list, a plain hash map) each fail to deliver O(1) on both get and put — the elegant solution requires COMBINING two structures.",
        },
        {
          tag: "p",
          text: "The combination is a hash map (for O(1) key lookup) plus a doubly linked list (for O(1) reordering and eviction). The hash map stores key → node pointer, letting you jump directly to any node without traversal. The doubly linked list maintains usage-recency order, with the most-recently-used node kept at one end and the least-recently-used at the other, ready for instant eviction.",
        },
        ...
        },
        {
          tag: "note",
          variant: "tip",
          text: "Why not just a hash map plus a plain array sorted by recency? Because reordering an array on every access requires shifting elements — O(n). The doubly linked list is what makes moving a node to the front (or back) genuinely O(1), with no shifting required.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Both get and put always perform a fixed, small number of pointer operations and a single hash map lookup, regardless of cache contents — there's no notion of a 'luckier' input producing faster results, since the operations are structurally constant-time.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Both operations rely on hash map average-case O(1) lookup (assuming a reasonable hash function and load factor) combined with strictly O(1) doubly-linked-list pointer rewiring — there's no scenario where the cache's current size or access pattern changes this.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "The only way this degrades is via hash map collisions — a pathologically bad hash function or adversarial key set could push hash map operations to O(n) in the absolute worst case, though this is not characteristic of normal use with a well-implemented hash function.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "The cache stores at most 'capacity' entries by design, requiring space proportional to the cache's configured capacity for both the hash map entries and the linked list nodes.",
          },
          {
            tag: "ul",
            items: [
              "Hash map: up to capacity key → node-pointer entries — O(n)",
              "Doubly linked list: up to capacity nodes — O(n)",
            ],
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "Space usage is bounded by the fixed cache capacity, which is a configuration parameter, not something that grows unpredictably with access patterns.",
          },
          {
            tag: "ul",
            items: [
              "Both structures stay capped at O(n), where n is the configured capacity, regardless of how many get/put operations are performed over the cache's lifetime",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "No access pattern increases space beyond the fixed capacity — eviction guarantees the cache never grows past its configured size limit, even under continuous insertion of new keys.",
          },
          {
            tag: "ul",
            items: [
      ...
      pseudoCodeandStepexplanation: [
        ...
        {
          tag: "p",
          text: "Using a doubly linked list with dummy head/tail sentinels, keeping most-recently-used near the head:",
        },
        {
          tag: "code",
          language: "text",
    ...
    function insertAtFront(node):
        ...
        {
          tag: "p",
          text: "Invariant: the doubly linked list is always ordered from most-recently-used (right after head) to least-recently-used (right before tail). Every get or put that touches an existing key unlinks and re-inserts it at the front, correctly re-establishing it as most-recently-used — and since unlink/re-link only touches the node's immediate neighbors (O(1) pointer rewiring, made possible because the hash map gives a direct node reference, avoiding any traversal), the recency ordering invariant is preserved on every operation. When eviction is needed, the node just before tail is, by the maintained invariant, guaranteed to be the genuinely least-recently-used entry in the entire cache.",
        },
      ],
      codes: {
        "c++": `#include <iostream>
...
fn main() {
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "Reversing a singly linked list flips the direction of every next pointer, so the list traverses in the opposite order — what was the head becomes the new tail, and what was the tail becomes the new head. It can be done iteratively (walking through once, rewiring pointers as you go) or recursively (reversing the rest of the list first, then fixing up the current node's links).",
        },
        {
          tag: "p",
          text: "It's the quintessential linked-list pointer-manipulation exercise, and a common building block inside larger problems: reversing a sub-portion of a list, checking for palindrome structure, or reordering a list in a specific pattern (e.g. L0→Ln→L1→Ln-1→...) all rely on reversal as a sub-step.",
        },
        { tag: "h2", text: "When to reach for it" },
        {
          tag: "ul",
        ...
        },
        {
          tag: "note",
          variant: "warning",
          text: "The single most common bug: overwriting current.next before saving a reference to it. Always capture the 'next' node into a temporary variable BEFORE reassigning current.next, or the rest of the original list becomes permanently unreachable.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Reversal must visit and rewire every single node's pointer — there's no early-exit shortcut, since every node's next pointer needs to be flipped for the reversal to be complete and correct.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Every node requires identical O(1) pointer-rewiring work regardless of the values stored in the list — there's no value-dependent branching in the traversal.",
          },
          {
            tag: "ul",
            items: [
              "n nodes × O(1) work each = O(n)",
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "No list configuration increases the cost beyond visiting every node exactly once — this is simultaneously the best, average, and worst case, since reversal has no input-dependent shortcuts at all.",
          },
          {
            tag: "ul",
            items: [
              "O(n) holds unconditionally",
              "This matches the trivial lower bound: every node's pointer must be examined and modified at least once for a correct reversal",
            ],
          },
        ],
      },
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "The iterative approach reverses pointers in-place, needing only a constant number of pointer variables to track the current, previous, and next nodes during traversal.",
          },
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Memory usage never depends on list length or content for the iterative approach — it's always exactly three pointer variables.",
          },
          {
            tag: "ul",
            items: [
              "No auxiliary array or new nodes are created — purely in-place pointer rewiring",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "Even reversing the longest possible list uses no more than the fixed three pointer variables for the iterative version — though the recursive version's call stack grows with list length.",
          },
          {
            tag: "ul",
            items: [
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        },
        {
          tag: "note",
          variant: "tip",
          text: "Sentinel (dummy) head and tail nodes are the standard production technique for doubly linked lists — they eliminate every null-check special case for 'is this the first/last node', at the small fixed cost of two extra permanent nodes.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Insertion or deletion AT A KNOWN NODE (given a direct reference, not found via search) is genuinely O(1) — this is the entire structural advantage doubly linked lists offer over arrays for this specific operation.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "When the operation requires first FINDING a node by value (rather than already having a direct reference), the search itself dominates the cost, since a doubly linked list offers no faster-than-linear search regardless of direction.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "If the target node is at the far end of the list relative to the traversal's starting point, the search (and thus the overall find-and-modify operation) costs O(n).",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Each node requires storage for its value plus two pointers (next and prev) — double the pointer overhead of a singly linked list, but still linear in the number of nodes.",
          },
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Space usage is fixed by node count alone, since every node has exactly the same fixed-size structure regardless of its position or the values stored in the list.",
          },
          {
            tag: "ul",
            items: [
              "Same O(n) bound regardless of value distribution or insertion/deletion history",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "No sequence of operations increases per-node overhead beyond the fixed two-pointer structure — space scales linearly and predictably with the current node count.",
          },
          {
            tag: "ul",
            items: [
              "O(n) total, plus a small constant for optional sentinel head/tail nodes if used",
            ],
          },
        ],
      },
      ...
      pseudoCodeandStepexplanation: [
        { tag: "h1", text: "Pseudocode & Step-by-Step Explanation" },
        {
          tag: "code",
          language: "text",
          text: `function insertAfter(node, value):
...
fn main() {
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "Its elegance is achieving O(1) auxiliary space — a hash-set-based approach (recording every visited node and checking for repeats) would also detect a cycle correctly, but at O(n) space cost. The two-pointer trick gets the same O(n) time with no extra memory at all, beyond two pointer variables.",
        },
        { tag: "h2", text: "When to reach for it" },
        {
          tag: "ul",
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "If the list is extremely short with no cycle (e.g. zero or one node), the fast pointer reaches null almost immediately, terminating the algorithm very quickly.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "For a list with no cycle, the fast pointer (moving twice as fast as slow) reaches the end in roughly n/2 steps. For a list WITH a cycle, the two pointers are guaranteed to meet within at most one full cycle length after the slow pointer enters the cycle.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "Even in the least favourable cycle configuration (e.g. a very long 'tail' before a short cycle), the total number of steps before detection remains linear in the total number of nodes.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Only two pointer variables (slow and fast) are needed throughout the entire algorithm, regardless of list length or cycle presence.",
          },
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Memory usage never grows with list length — it's always exactly two pointer variables, a stark contrast to the O(n) space a hash-set-based cycle detection approach would require.",
          },
          {
            tag: "ul",
            items: ["No auxiliary set, array, or recursion stack — purely two traversal pointers"],
          },
        ],
        worst: [
          { tag: "h2", text: "Worst Case Space — O(1)" },
          {
            tag: "p",
            text: "Even the longest possible list with the most elaborate cycle structure doesn't require any additional memory beyond the two fixed pointers.",
          },
          {
            tag: "ul",
            items: [
...
fn main() {
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "Given two (or more) already-sorted linked lists, merging combines them into a single sorted linked list, in-place, by repeatedly comparing the front nodes of each list and splicing the smaller one onto the result — without ever allocating a new node, only rewiring existing next pointers.",
        },
        {
          tag: "p",
          text: "This is exactly the merge step from Merge Sort, but adapted to linked lists' pointer-rewiring nature instead of arrays' index-copying nature — and it's actually SIMPLER on a linked list, since there's no need for a separate auxiliary buffer the way array-based merging requires; nodes are just relinked directly into their final positions.",
        },
        { tag: "h2", text: "When to reach for it" },
        {
          tag: "ul",
        ...
        },
        {
          tag: "note",
          variant: "tip",
          text: "Using a dummy/sentinel head node for the result list eliminates the special case of 'what is the very first node of the merged result' — you simply return dummy.next at the end, regardless of which input list contributed the actual first node.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        average: [
          ...
          {
            tag: "p",
            text: "For interleaved sorted values, the comparison loop must process nodes from both lists roughly proportionally, until one list is exhausted, after which the remainder of the other is spliced on directly.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "The worst case — values perfectly interleaved between the two lists (alternating which list contributes the smaller value at every step) — still results in exactly n + m total node visits, since merging always processes every node exactly once regardless of interleaving pattern.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "The merge operates entirely by relinking existing nodes' next pointers — no new nodes are ever allocated, only a few pointer variables to track the current position in each list and the result.",
          },
          {
            tag: "ul",
            items: ["dummy/result tail pointer, plus pointers into each input list — O(1)"],
          },
        ],
        average: [
          { tag: "h2", text: "Average Case Space — O(1)" },
          {
            tag: "p",
            text: "Memory usage never depends on the number of nodes being merged — it's always exactly a fixed handful of pointer variables, regardless of how the values interleave.",
          },
          {
            tag: "ul",
            items: [
              "No auxiliary array or new node allocation, unlike array-based Merge Sort's O(n) auxiliary buffer requirement",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "No interleaving pattern or list length increases memory usage beyond the fixed set of tracking pointers — this is a genuine structural advantage of linked-list merging over array-based merging.",
          },
          {
            tag: "ul",
            items: [
      ...
      pseudoCodeandStepexplanation: [
        { tag: "h1", text: "Pseudocode & Step-by-Step Explanation" },
        {
          tag: "code",
          language: "text",
          text: `function mergeTwoLists(l1, l2):
