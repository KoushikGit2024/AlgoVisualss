const GREEDY_SECTION = {
  ...
  about: [
    ...
    { tag: "h2", text: "Greedy vs. Dynamic Programming" },
    {
      ...
      rows: [
        [
          "Choices reconsidered?",
          "Never — each choice is final",
          "Implicitly, via exploring all subproblem combinations",
        ],
        [
          "Typical complexity",
          "O(n log n) (often dominated by an initial sort)",
          "O(n²) or worse (state space exploration)",
        ],
        [
          "Requires proof of correctness?",
          "Yes — greedy choice property must be explicitly justified",
          "Optimal substructure alone is usually sufficient",
        ],
        [
          "Classic warning example",
          "0/1 Knapsack — greedy by value/weight ratio fails",
          "Coin Change with non-canonical coins — greedy fails, DP succeeds",
        ],
      ],
    },
    ...
    },
    {
      tag: "note",
      variant: "warning",
      text: "Greedy algorithms have no general-purpose 'try it and see' safety net — getting the greedy choice property wrong produces a confidently wrong answer, not a slow-but-correct one. Always verify the exchange argument (or find it already proven, as with the classic problems in this section) before trusting a greedy approach on a new problem.",
    },
  ],
  ...
  items: [
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "Given an array where each element represents the MAXIMUM jump length from that position, determine whether it's possible to reach the last index starting from the first. A naive backtracking/DP approach explores every possible sequence of jumps, but a greedy approach solves it in a single O(n) pass by tracking only the FARTHEST reachable index seen so far, without ever needing to know the specific sequence of jumps that gets there.",
        },
        {
          tag: "p",
          text: "The greedy insight: at every position, all that matters for determining reachability of the END is the single number 'farthest index reachable so far' — the SPECIFIC path taken to achieve that reach is irrelevant, since any jump sequence achieving the same maximum reach is equally good for the purpose of eventually reaching the end. This collapses what looks like an exponential path-exploration problem into tracking one running value.",
        },
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "If the very first element's jump length already reaches (or exceeds) the last index, the algorithm can determine success after examining just one element.",
          },
          {
            tag: "ul",
            items: [
              "arr[0] >= n − 1: immediately confirmed reachable — O(1)",
        ...
        average: [
          ...
          {
            tag: "p",
            text: "A single forward pass examines each position once, updating the running 'farthest reachable' value with an O(1) comparison at each step.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "Even when the answer is 'unreachable' (requiring confirmation by reaching a position beyond the current farthest reach), the algorithm need only scan up to that failure point — bounded by O(n) in any case.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Only a single running variable (the farthest reachable index so far) is needed throughout the entire algorithm.",
          },
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Memory usage never depends on array length or content — it's always exactly one running variable, a stark contrast to a DP-based approach that would typically need an O(n) array tracking reachability at every position.",
          },
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "No input configuration increases memory usage beyond the single tracked variable — this is the entire structural advantage of the greedy approach over a DP-based alternative for this specific problem.",
          },
          {
            tag: "ul",
            items: [
              "O(1) regardless of n — this space efficiency is a direct consequence of needing to track only the SINGLE most useful summary statistic (farthest reach), rather than full per-position state",
            ],
          },
        ],
      },
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Building the tree always requires exactly n − 1 merge operations (reducing n initial symbol-nodes down to a single root), each involving two O(log n) heap extractions and one O(log n) heap insertion — there's no shortcut even for the most favourable frequency distribution.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Every merge operation performs the same fixed sequence of heap operations regardless of the specific frequency VALUES involved — the structural cost is determined entirely by the number of symbols n, not by how frequencies are distributed among them.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "No frequency distribution increases the cost beyond the fixed n − 1 heap-based merges — this is simultaneously the best, average, and worst case, since the tree-building process always performs exactly the same number of structurally identical merge steps regardless of symbol frequencies.",
          },
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "The min-heap initially holds n symbol-frequency pairs, and the final tree contains n leaf nodes (one per symbol) plus n − 1 internal merge nodes — both bounded by O(n).",
          },
          {
            tag: "ul",
            items: [
              "Min-heap: O(n)",
              "Final Huffman tree: 2n − 1 total nodes (n leaves + n−1 internal) — O(n)",
            ],
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "Space usage is fixed by the number of distinct symbols n alone, regardless of how their frequencies are distributed or how 'balanced' the resulting tree ends up being.",
          },
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "No frequency distribution increases space beyond the fixed 2n − 1 total tree nodes — this holds even for the most skewed possible frequency distribution (e.g. Fibonacci-like frequencies, which produce a maximally unbalanced, linked-list-shaped tree, but still with exactly 2n − 1 total nodes).",
          },
          {
            tag: "ul",
            items: [
              "O(n) total, identical across all cases — tree SHAPE (balanced vs. skewed) varies with frequency distribution, but total node COUNT never does",
            ],
          },
        ],
      },
      ...
      pseudoCodeandStepexplanation: [
        { tag: "h1", text: "Pseudocode & Step-by-Step Explanation" },
        {
          tag: "code",
          language: "text",
          text: `function buildHuffmanTree(symbols, frequencies):
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        },
        {
          tag: "note",
          variant: "tip",
          text: "For the full step-by-step pseudocode and complexity breakdown of the two concrete MST algorithms, see Kruskal's Algorithm and Prim's Algorithm in the Graphs section — this entry focuses specifically on the shared greedy PRINCIPLE (the Cut Property) that proves both of them correct.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Both standard MST algorithms always process the full edge set at least once (Kruskal's via sorting, Prim's via priority-queue-driven exploration) regardless of how favourably the graph's weights happen to be arranged — there's no shortcut even for the most structurally simple graph.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Both algorithms perform the same fixed structural operations (sorting plus union-find for Kruskal's; heap extraction and decrease-key for Prim's) regardless of the SPECIFIC edge weight values, only their relative ordering affects which edges get chosen, not the total operation count.",
          },
          {
            tag: "ul",
            items: [
              "Both algorithms: O(E log V) (or equivalently O(E log E)), as detailed individually in the Graphs section's Kruskal's Algorithm and Prim's Algorithm entries",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "No edge weight configuration increases the cost beyond the standard sorting-or-heap-based bound for either algorithm — this is the conventionally cited bound for general-purpose MST construction, with the specific better choice between Kruskal's and Prim's depending on graph density (sparse favours Kruskal's, dense favours Prim's with a Fibonacci heap).",
          },
          {
            tag: "ul",
            items: [
              "O(E log V) is the standard combined classification for both algorithms; see the Graphs section for the precise, algorithm-specific worst-case analysis of each",
            ],
          },
        ],
      },
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Both algorithms need to represent the full graph (O(V + E) for an adjacency-list representation) plus algorithm-specific auxiliary structures (Union-Find for Kruskal's, a priority queue and key array for Prim's), as detailed in the Graphs section.",
          },
          {
            tag: "ul",
            items: [
              "Graph representation: O(V + E)",
              "Auxiliary structures (Union-Find or priority queue + key array): O(V), as detailed per-algorithm in the Graphs section",
            ],
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "Space usage is fixed by graph size alone for both algorithms, since neither's auxiliary structures scale with edge weight values or the specific MST that ends up being constructed.",
          },
          {
            tag: "ul",
            items: [
              "Same O(V + E) bound for both algorithms, regardless of edge weight distribution",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "No graph configuration increases space beyond the standard graph representation plus fixed-size auxiliary structures — see the Graphs section's Kruskal's Algorithm and Prim's Algorithm entries for the precise per-algorithm breakdown.",
          },
          {
            tag: "ul",
            items: ["O(V + E) total, identical across all cases for both standard MST algorithms"],
          },
        ],
      },
      ...
      pseudoCodeandStepexplanation: [
        ...
        {
          tag: "p",
          text: "This entry focuses on the shared CUT PROPERTY proof rather than re-deriving either algorithm's full pseudocode (see Kruskal's Algorithm / Prim's Algorithm in the Graphs section for those):",
        },
        {
          tag: "code",
          language: "text",
          text: `// The Cut Property (the shared justification for BOTH greedy MST algorithms):
//
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        },
        {
          tag: "note",
          variant: "warning",
          text: "A common mistake is sorting by activity DURATION (shortest first) instead of finish time — this is a different greedy rule that does NOT have the same optimality guarantee and can produce a suboptimal (fewer than maximum) selection on many inputs. Finish time, specifically, is the proven-correct sort key.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Sorting the activities by finish time always dominates the cost and is required regardless of input arrangement — there's no shortcut even if the activities happen to already be in a favourable order, since a comparison-based sort is always at least Θ(n log n).",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Both the sorting step and the selection pass perform the same structural work regardless of the specific start/finish time values — comparison-based sorting is Θ(n log n) for any input, and the selection pass is always a single O(n) scan.",
          },
          {
            tag: "ul",
            items: ["O(n log n) for sorting (dominates) + O(n) for selection = O(n log n)"],
          },
        ],
        worst: [
          { tag: "h2", text: "Worst Case — O(n log n)" },
          {
            tag: "p",
            text: "No arrangement of start/finish times increases the cost beyond the sorting step's bound — this is simultaneously the best, average, and worst case.",
          },
          {
            tag: "ul",
            items: [
              "Worst case identical to best/average: O(n log n)",
              "If the activities are GIVEN already pre-sorted by finish time, the sort can be skipped entirely, reducing the bound to O(n) — but this is a precondition on the input, not something the general algorithm can assume",
            ],
          },
        ],
      },
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Beyond the space needed for sorting (which can often be done in-place, or O(log n) for typical sort implementations' recursion stack), only a single variable tracking the most recently selected activity's finish time is needed.",
          },
          {
            tag: "ul",
            items: [
              "lastFinishTime — O(1) auxiliary tracking, beyond whatever the sorting step itself requires",
            ],
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "The selection pass itself uses no auxiliary space proportional to n — it's a single forward scan with one tracked variable, regardless of how many activities are ultimately selected.",
          },
          {
            tag: "ul",
            items: [
              "O(1) for the selection logic itself; total space is dominated by whatever the sorting algorithm requires (commonly O(log n) for an in-place comparison sort's recursion stack, or O(n) for a non-in-place sort)",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "No input configuration increases the selection pass's auxiliary space beyond the single tracked finish-time variable — this holds regardless of how many activities end up being selected.",
          },
          {
            tag: "ul",
            items: [
              "O(1) for the core selection algorithm, identical across all cases (excluding whatever overhead the chosen sorting algorithm itself contributes)",
            ],
          },
        ],
      },
      ...
      pseudoCodeandStepexplanation: [
        { tag: "h1", text: "Pseudocode & Step-by-Step Explanation" },
        {
          tag: "code",
          language: "text",
          text: `function activitySelection(activities):           // each activity: (start, finish)
...
fn main() {
    ...
    ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "The greedy strategy: compute each item's value-to-weight RATIO, sort items by this ratio in descending order, and greedily take as much as possible of the highest-ratio item first, then the next, continuing until the knapsack's capacity is fully used. Since fractions are allowed, the LAST item considered can always be taken PARTIALLY to exactly fill any remaining capacity, with no 'wasted' leftover space — this is precisely the property that breaks for 0/1 Knapsack, where a partially-fitting item simply can't be used at all.",
        },
        { tag: "h2", text: "When to reach for it" },
        {
          tag: "ul",
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Sorting items by value-to-weight ratio always dominates the cost and is required regardless of input arrangement — there's no shortcut even if the items happen to already be in a favourable order, since a comparison-based sort is always at least Θ(n log n).",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Both the sorting step and the greedy fill pass perform the same structural work regardless of the specific weight/value values involved — sorting cost depends only on item count, and the fill pass is always a single O(n) scan.",
          },
          {
            tag: "ul",
            items: ["O(n log n) for sorting (dominates) + O(n) for the fill pass = O(n log n)"],
          },
        ],
        worst: [
          { tag: "h2", text: "Worst Case — O(n log n)" },
          {
            tag: "p",
            text: "No arrangement of weights and values increases the cost beyond the sorting step's bound — this is simultaneously the best, average, and worst case.",
          },
          {
            tag: "ul",
            items: [
              "Worst case identical to best/average: O(n log n)",
              "This is a dramatic improvement over 0/1 Knapsack's O(nW) pseudo-polynomial DP bound — achievable specifically because the divisibility property eliminates the need to explore a 2D state space",
            ],
          },
        ],
      },
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Beyond the space needed for sorting, only a small handful of running variables (remaining capacity, accumulated value) are needed for the greedy fill pass itself.",
          },
          {
            tag: "ul",
            items: [
              "remainingCapacity, totalValue — O(1) auxiliary tracking, beyond whatever the sorting step itself requires",
            ],
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "The greedy fill pass itself uses no auxiliary space proportional to n — it's a single forward scan with a couple of tracked running variables, regardless of how many items are ultimately used (fully or partially).",
          },
          {
            tag: "ul",
            items: [
              "O(1) for the core fill logic; total space is dominated by whatever the sorting algorithm requires (commonly O(log n) for an in-place comparison sort's recursion stack)",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "No input configuration increases the fill pass's auxiliary space beyond the fixed handful of running variables — this holds regardless of how the items' weights and values are distributed.",
          },
          {
            tag: "ul",
            items: [
              "O(1) for the core greedy algorithm, identical across all cases (excluding whatever overhead the chosen sorting algorithm itself contributes), a meaningful advantage over 0/1 Knapsack's O(nW) or O(W) space requirement",
            ],
          },
        ],
      },
      ...
      pseudoCodeandStepexplanation: [
        { tag: "h1", text: "Pseudocode & Step-by-Step Explanation" },
        {
          tag: "code",
          language: "text",
          text: `function fractionalKnapsack(items, capacity):     // each item: (weight, value)
    for item in items:
