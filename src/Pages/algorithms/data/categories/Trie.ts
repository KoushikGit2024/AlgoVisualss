const TRIES_SECTION = {
  ...
  about: [
    ...
    {
      tag: "p",
      text: "Every operation's complexity in this section is expressed in terms of m, the LENGTH of the word or prefix being processed — NOT n, the number of words stored in the trie. This is the trie's signature property: operations cost the same regardless of how many other words share the structure, because each operation only ever touches the single path corresponding to its own characters, never any other stored word's nodes.",
    },
    {
      tag: "note",
      variant: "tip",
      text: "If a problem mentions 'autocomplete', 'prefix matching', or searching with wildcard characters across many candidate words simultaneously, a trie is almost always the right structure — these are exactly the operations a trie is purpose-built for.",
    },
  ],
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        },
        {
          tag: "note",
          variant: "tip",
          text: "A key optimisation: once a word is found during the grid DFS, mark its trie node so it can't be matched again (e.g. clear its 'is end of word' flag), and optionally prune trie leaves with no children after a word is found — this prevents re-discovering the same word via multiple paths and keeps the trie from doing wasted work on already-found words.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Building the trie always requires inserting every word fully, and the grid DFS always starts from every one of the m grid cells regardless of how favourably the words and grid happen to align — there's no shortcut even for the most prunable input.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "No specific grid or word list increases the asymptotic classification beyond this conventional bound, representing the structural ceiling of the combined trie-pruned DFS search across the whole grid.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "The trie requires space proportional to the total number of distinct characters across all inserted words (with shared prefixes contributing to space only once, not once per word), plus the grid DFS's recursion stack bounded by the maximum word length.",
          },
          {
            tag: "ul",
            items: [
              "Trie: O(N), where N is the total character count across all words after accounting for shared-prefix compression",
              "DFS recursion stack: O(L), the maximum word length",
            ],
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "Trie space is fixed by the total distinct-prefix character count across the word list, which can be substantially smaller than the sum of all individual word lengths when many words share common prefixes.",
          },
          {
            tag: "ul",
            items: [
              "Trie size scales with N, the total UNIQUE prefix-path character count, not the raw sum of word lengths — a meaningful space saving for word lists with significant prefix overlap",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "If every word shares no characters in common with any other (no prefix overlap at all), the trie's size approaches the raw sum of all word lengths, its structural maximum.",
          },
          {
            tag: "ul",
            items: [
      ...
      pseudoCodeandStepexplanation: [
        { tag: "h1", text: "Pseudocode & Step-by-Step Explanation" },
        {
          tag: "code",
          language: "text",
          text: `function findWords(board, words):
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Every trie operation must walk the full length of the word or prefix being processed — there's no shortcut even for the most favourable trie structure, since correctness requires confirming every character along the path.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Every operation performs exactly one O(1) child-lookup per character of the word being processed, regardless of how many other words are already stored in the trie or how deeply nested the relevant path is.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "No trie size or word content increases the cost beyond the fixed per-character walk — this is simultaneously the best, average, and worst case, since trie traversal has no value-dependent branching or backtracking.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "If inserting a word that's entirely a prefix of an ALREADY-inserted longer word (or shares its entire path with an existing word), no new nodes need to be created at all — only the existing end node's isEndOfWord flag needs to be set.",
          },
          {
            tag: "ul",
            items: [
              "If the word's full path already exists in the trie: O(1) additional space (just flipping a flag)",
            ],
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "Inserting a word requires creating new nodes only for the portion of its path that DOESN'T already exist in the trie — for typical word lists with some but not complete prefix overlap, this is some fraction of m.",
          },
          {
            tag: "ul",
            items: [
              "New nodes created: up to O(m), depending on how much of the word's prefix path already exists from previously inserted words",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "If a word shares NO prefix at all with any previously inserted word, every one of its m characters requires a brand new node.",
          },
          {
            tag: "ul",
            items: [
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        },
        {
          tag: "note",
          variant: "tip",
          text: "Without any wildcards in the query, this collapses to the exact same O(m) single-path walk as plain Implement Trie's search() — the wildcard-driven branching only kicks in at positions where a '.' actually appears in the query string.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "A query with NO wildcard characters behaves exactly like a plain trie search — a single deterministic O(m) path walk, with no branching at all.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "If EVERY character in the query is a wildcard (e.g. search('....') for a 4-letter word), the search must explore every possible branch at every position, growing exponentially with the query length in the absolute worst case.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "For a query with no wildcards, the search uses only a single recursion depth of m (or O(1) extra space if implemented iteratively), matching plain trie search exactly.",
          },
          {
            tag: "ul",
            items: [
              "Recursion stack depth: O(m), bounded by the query length, regardless of wildcard count",
            ],
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "Even with multiple wildcard branches being explored, the recursion depth at any single point in time is still bounded by the query length m — branching increases the NUMBER of recursive calls made overall, not the maximum depth of any single call chain.",
          },
          {
            tag: "ul",
            items: [
              "Recursion stack: O(m), since the DFS exploring wildcard branches still only ever recurses to a depth matching the query's remaining length, regardless of how many sibling branches exist at each wildcard position",
            ],
          },
        ],
        worst: [
          ...
          {
            tag: "p",
            text: "Even in the worst case of a fully-wildcard query triggering maximal branching, the recursion stack at any given moment is still bounded by the query length, since branches are explored one at a time (depth-first), not all simultaneously in parallel.",
          },
          {
            tag: "ul",
            items: [
