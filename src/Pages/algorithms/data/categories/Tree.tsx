...
// Allocates a new leaf node (no children yet).
...
// Useful for serialising a tree, since the root is always written first.
...
// For a Binary Search Tree, this always visits values in ascending order.
...
// Safe for deleting a tree node-by-node, since children are freed before parents.
...
// Level-order (breadth-first): visits the tree one depth layer at a time,
...
int main() {
    // Build a small, fixed sample tree:
    //             3
    //           /   \\
    //          5      1
    //        /  \\    / \\
...
def pre_order_traversal(node, output):
    """Pre-order: Node -> Left -> Right.
    Useful for serialising a tree, since the root is always written first."""
    if node is None:
        return
...
def post_order_traversal(node, output):
    """Post-order: Left -> Right -> Node.
    Safe for deleting a tree node-by-node, since children are freed before parents."""
    if node is None:
        return
...
def level_order_traversal(root, output):
    """Level-order (breadth-first): visits the tree one depth layer at a time,
    using a queue instead of recursion."""
    if root is None:
        return
...
if __name__ == "__main__":
    ...
    print("Pre-order:  ", " ".join(str(v) for v in pre_order_result))

    in_order_result = []
    in_order_traversal(root, in_order_result)
    print("In-order:   ", " ".join(str(v) for v in in_order_result))

    post_order_result = []
    post_order_traversal(root, post_order_result)
    print("Post-order: ", " ".join(str(v) for v in post_order_result))
...
public class TreeTraversals {
    ...
    // Pre-order: Node -> Left -> Right.
    // Useful for serialising a tree, since the root is always written first.
    static void preOrderTraversal(TreeNode node, StringBuilder output) {
        if (node == null) return;
        output.append(node.value).append(" ");
    ...
    // In-order: Left -> Node -> Right.
    // For a Binary Search Tree, this always visits values in ascending order.
    static void inOrderTraversal(TreeNode node, StringBuilder output) {
        if (node == null) return;
    ...
    // Post-order: Left -> Right -> Node.
    // Safe for deleting a tree node-by-node, since children are freed before parents.
    static void postOrderTraversal(TreeNode node, StringBuilder output) {
        if (node == null) return;
    ...
    // Level-order (breadth-first): visits the tree one depth layer at a time,
    // using a queue instead of recursion.
    static void levelOrderTraversal(TreeNode root, StringBuilder output) {
        if (root == null) return;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Pre-order:   " + preOrderOutput.toString().trim());

        StringBuilder inOrderOutput = new StringBuilder();
        inOrderTraversal(root, inOrderOutput);
        System.out.println("In-order:    " + inOrderOutput.toString().trim());

        StringBuilder postOrderOutput = new StringBuilder();
        postOrderTraversal(root, postOrderOutput);
        System.out.println("Post-order:  " + postOrderOutput.toString().trim());

        StringBuilder levelOrderOutput = new StringBuilder();
        levelOrderTraversal(root, levelOrderOutput);
        System.out.println("Level-order: " + levelOrderOutput.toString().trim());
    }
}
`,
...
// Level-order (breadth-first): visits the tree one depth layer at a time,
// using a queue instead of recursion.
function levelOrderTraversal(root, output) {
    if (root === null) return;
...
// A simple fixed-capacity queue of TreeNode pointers, used for level-order
// traversal (no dynamic-size queue is available in plain C).
...
void levelOrderTraversal(TreeNode* root) {
    ...
    TreeNode* queueStorage[MAX_QUEUE_SIZE];
    int frontIndex = 0;
    int backIndex = 0;

    queueStorage[backIndex++] = root;

    while (frontIndex < backIndex) {
        TreeNode* current = queueStorage[frontIndex++];
        printf("%d ", current->value);

        if (current->left != NULL) queueStorage[backIndex++] = current->left;
        if (current->right != NULL) queueStorage[backIndex++] = current->right;
    }
}
...
public static class TreeTraversals {
    ...
    public static void Main() {
        ...
        var preOrderOutput = new StringBuilder();
        PreOrderTraversal(root, preOrderOutput);
        Console.WriteLine("Pre-order:   " + preOrderOutput.ToString().Trim());

        var inOrderOutput = new StringBuilder();
        InOrderTraversal(root, inOrderOutput);
        Console.WriteLine("In-order:    " + inOrderOutput.ToString().Trim());

        var postOrderOutput = new StringBuilder();
        PostOrderTraversal(root, postOrderOutput);
        Console.WriteLine("Post-order:  " + postOrderOutput.ToString().Trim());

        var levelOrderOutput = new StringBuilder();
        LevelOrderTraversal(root, levelOrderOutput);
        Console.WriteLine("Level-order: " + levelOrderOutput.ToString().Trim());
    }
}
`,
        "swift": `import Foundation
...
// Pre-order: Node -> Left -> Right.
// Useful for serialising a tree, since the root is always written first.
func preOrderTraversal(_ node: TreeNode?, _ output: inout [Int]) {
    guard let node = node else { return }
    output.append(node.value)
    preOrderTraversal(node.left, &output)
    preOrderTraversal(node.right, &output)
...
// In-order: Left -> Node -> Right.
// For a Binary Search Tree, this always visits values in ascending order.
func inOrderTraversal(_ node: TreeNode?, _ output: inout [Int]) {
    guard let node = node else { return }
    inOrderTraversal(node.left, &output)
    output.append(node.value)
    inOrderTraversal(node.right, &output)
...
// Post-order: Left -> Right -> Node.
// Safe for deleting a tree node-by-node, since children are freed before parents.
func postOrderTraversal(_ node: TreeNode?, _ output: inout [Int]) {
    guard let node = node else { return }
    postOrderTraversal(node.left, &output)
    postOrderTraversal(node.right, &output)
    output.append(node.value)
...
// Level-order (breadth-first): visits the tree one depth layer at a time,
// using a queue instead of recursion.
func levelOrderTraversal(_ root: TreeNode?, _ output: inout [Int]) {
    guard let root = root else { return }

    var nodeQueue = [root]
    var frontIndex = 0 // index-based dequeue to avoid O(n) removeFirst

    while frontIndex < nodeQueue.count {
        let current = nodeQueue[frontIndex]
        frontIndex += 1
        output.append(current.value)

        if let left = current.left { nodeQueue.append(left) }
        if let right = current.right { nodeQueue.append(right) }
    }
}
...
preOrderTraversal(root, &preOrderResult)
print("Pre-order:   \(preOrderResult.map { String($0) }.joined(separator: " "))")

var inOrderResult: [Int] = []
inOrderTraversal(root, &inOrderResult)
print("In-order:    \(inOrderResult.map { String($0) }.joined(separator: " "))")

var postOrderResult: [Int] = []
postOrderTraversal(root, &postOrderResult)
print("Post-order:  \(postOrderResult.map { String($0) }.joined(separator: " "))")

var levelOrderResult: [Int] = []
levelOrderTraversal(root, &levelOrderResult)
...
// Pre-order: Node -> Left -> Right.
// Useful for serialising a tree, since the root is always written first.
fun preOrderTraversal(node: TreeNode?, output: MutableList<Int>) {
    if (node == null) return
    output.add(node.value)
    preOrderTraversal(node.left, output)
    preOrderTraversal(node.right, output)
...
// In-order: Left -> Node -> Right.
// For a Binary Search Tree, this always visits values in ascending order.
fun inOrderTraversal(node: TreeNode?, output: MutableList<Int>) {
    if (node == null) return
    inOrderTraversal(node.left, output)
    output.add(node.value)
    inOrderTraversal(node.right, output)
...
// Post-order: Left -> Right -> Node.
// Safe for deleting a tree node-by-node, since children are freed before parents.
fun postOrderTraversal(node: TreeNode?, output: MutableList<Int>) {
    if (node == null) return
    postOrderTraversal(node.left, output)
    postOrderTraversal(node.right, output)
    output.add(node.value)
...
// Level-order (breadth-first): visits the tree one depth layer at a time,
// using a queue instead of recursion.
fun levelOrderTraversal(root: TreeNode?, output: MutableList<Int>) {
    if (root == null) return

    val nodeQueue: java.util.Queue<TreeNode> = LinkedList()
    nodeQueue.add(root)
    ...
    while (nodeQueue.isNotEmpty()) {
        ...
        current.left?.let { nodeQueue.add(it) }
        current.right?.let { nodeQueue.add(it) }
    }
}
...
fun main() {
    ...
    preOrderTraversal(root, preOrderResult)
    println("Pre-order:   \${preOrderResult.joinToString(" ")}")

    val inOrderResult = mutableListOf<Int>()
    inOrderTraversal(root, inOrderResult)
    println("In-order:    \${inOrderResult.joinToString(" ")}")

    val postOrderResult = mutableListOf<Int>()
    postOrderTraversal(root, postOrderResult)
    println("Post-order:  \${postOrderResult.joinToString(" ")}")
...
object TreeTraversals extends App {
    ...
    // Pre-order: Node -> Left -> Right.
    // Useful for serialising a tree, since the root is always written first.
    def preOrderTraversal(node: TreeNode, output: mutable.ListBuffer[Int]): Unit = {
        if (node == null) return
        output += node.value
        preOrderTraversal(node.left, output)
        preOrderTraversal(node.right, output)
    ...
    // In-order: Left -> Node -> Right.
    // For a Binary Search Tree, this always visits values in ascending order.
    def inOrderTraversal(node: TreeNode, output: mutable.ListBuffer[Int]): Unit = {
        if (node == null) return
        inOrderTraversal(node.left, output)
        output += node.value
        inOrderTraversal(node.right, output)
    ...
    // Post-order: Left -> Right -> Node.
    // Safe for deleting a tree node-by-node, since children are freed before parents.
    def postOrderTraversal(node: TreeNode, output: mutable.ListBuffer[Int]): Unit = {
        if (node == null) return
        postOrderTraversal(node.left, output)
        postOrderTraversal(node.right, output)
        output += node.value
    ...
    // Level-order (breadth-first): visits the tree one depth layer at a time,
    // using a queue instead of recursion.
    def levelOrderTraversal(root: TreeNode, output: mutable.ListBuffer[Int]): Unit = {
        if (root == null) return

        val nodeQueue = mutable.Queue[TreeNode]()
        nodeQueue.enqueue(root)
    ...
    val preOrderResult = mutable.ListBuffer[Int]()
    preOrderTraversal(root, preOrderResult)
    println(s"Pre-order:   \${preOrderResult.mkString(" ")}")

    val inOrderResult = mutable.ListBuffer[Int]()
    inOrderTraversal(root, inOrderResult)
    println(s"In-order:    \${inOrderResult.mkString(" ")}")

    val postOrderResult = mutable.ListBuffer[Int]()
    postOrderTraversal(root, postOrderResult)
    println(s"Post-order:  \${postOrderResult.mkString(" ")}")

    val levelOrderResult = mutable.ListBuffer[Int]()
    levelOrderTraversal(root, levelOrderResult)
...
// TreeNode is a minimal binary tree node used to demonstrate all four
// traversal orders.
...
// preOrderTraversal visits Node -> Left -> Right.
// Useful for serialising a tree, since the root is always written first.
...
// inOrderTraversal visits Left -> Node -> Right.
// For a Binary Search Tree, this always visits values in ascending order.
func inOrderTraversal(node *TreeNode, output *[]int) {
    if node == nil {
        return
    }
    inOrderTraversal(node.left, output)
...
// postOrderTraversal visits Left -> Right -> Node.
// Safe for deleting a tree node-by-node, since children are freed before parents.
func postOrderTraversal(node *TreeNode, output *[]int) {
    if node == nil {
        return
    }
    postOrderTraversal(node.left, output)
...
// levelOrderTraversal (breadth-first) visits the tree one depth layer at a
// time, using a queue instead of recursion.
func levelOrderTraversal(root *TreeNode, output *[]int) {
    if root == nil {
        return
    }
...
func main() {
    ...
    var preOrderResult []int
    preOrderTraversal(root, &preOrderResult)
    fmt.Println("Pre-order:  ", preOrderResult)

    var inOrderResult []int
    inOrderTraversal(root, &inOrderResult)
    fmt.Println("In-order:   ", inOrderResult)

    var postOrderResult []int
    postOrderTraversal(root, &postOrderResult)
    fmt.Println("Post-order: ", postOrderResult)
...
/// A minimal binary tree node used to demonstrate all four traversal orders.
/// Rc<RefCell<..>> is used so multiple owners (parent references during
/// construction, and the queue during level-order) can share nodes safely.
struct TreeNode {
    value: i32,
...
/// Pre-order: Node -> Left -> Right.
/// Useful for serialising a tree, since the root is always written first.
fn pre_order_traversal(node: &Option<Rc<RefCell<TreeNode>>>, output: &mut Vec<i32>) {
    if let Some(n) = node {
        let borrowed = n.borrow();
        output.push(borrowed.value);
        pre_order_traversal(&borrowed.left, output);
        pre_order_traversal(&borrowed.right, output);
    }
}

/// In-order: Left -> Node -> Right.
/// For a Binary Search Tree, this always visits values in ascending order.
fn in_order_traversal(node: &Option<Rc<RefCell<TreeNode>>>, output: &mut Vec<i32>) {
    if let Some(n) = node {
        let borrowed = n.borrow();
        in_order_traversal(&borrowed.left, output);
        output.push(borrowed.value);
        in_order_traversal(&borrowed.right, output);
    }
}

/// Post-order: Left -> Right -> Node.
/// Safe for deleting a tree node-by-node, since children are freed before parents.
fn post_order_traversal(node: &Option<Rc<RefCell<TreeNode>>>, output: &mut Vec<i32>) {
    if let Some(n) = node {
        let borrowed = n.borrow();
        post_order_traversal(&borrowed.left, output);
        post_order_traversal(&borrowed.right, output);
        output.push(borrowed.value);
    }
}
...
/// Level-order (breadth-first): visits the tree one depth layer at a time,
/// using a queue instead of recursion.
fn level_order_traversal(root: &Option<Rc<RefCell<TreeNode>>>, output: &mut Vec<i32>) {
...
fn main() {
    ...
    left.borrow_mut().left = Some(TreeNode::new(6));
    let left_right = TreeNode::new(2);
    left_right.borrow_mut().left = Some(TreeNode::new(7));
    left_right.borrow_mut().right = Some(TreeNode::new(4));
    left.borrow_mut().right = Some(left_right);

    right.borrow_mut().left = Some(TreeNode::new(0));
    right.borrow_mut().right = Some(TreeNode::new(8));
    ...
    pre_order_traversal(&root_option, &mut pre_order_result);
    println!("Pre-order:   {:?}", pre_order_result);

    let mut in_order_result = Vec::new();
    in_order_traversal(&root_option, &mut in_order_result);
    println!("In-order:    {:?}", in_order_result);

    let mut post_order_result = Vec::new();
    post_order_traversal(&root_option, &mut post_order_result);
    println!("Post-order:  {:?}", post_order_result);

    let mut level_order_result = Vec::new();
...
// Inserts 'value' into the BST rooted at 'node' and returns the (possibly
// new) root of this subtree. Duplicate values are silently ignored.
...
// Returns true if 'target' exists anywhere in the BST rooted at 'node'.
...
// Finds the node holding the smallest value in a subtree, by always
// following left-child pointers until none remain. Used during deletion
// of a node with two children (the in-order successor is the minimum of
// its right subtree).
...
// Deletes 'target' from the BST rooted at 'node' and returns the (possibly
// new) root of this subtree.
BSTNode* deleteNode(BSTNode* node, int target) {
    ...
    if (target < node->value) {
        ...
        if (node->right == nullptr) {
            BSTNode* replacement = node->left;
            delete node;
            return replacement;
        }
        // Two children: replace this node's value with its in-order
        // successor, then remove the successor from the right subtree.
...
int main() {
    ...
    // Delete a node with two children (30 has children 20 and 40)
    root = deleteNode(root, 30);
    cout << "After deleting 30: ";
    printInOrder(root);
    cout << endl;

    // Delete another node with two children (70 has children 60 and 80)
...
def insert(node, value):
    """Inserts 'value' into the BST rooted at 'node' and returns the
    (possibly new) root of this subtree. Duplicate values are ignored."""
    if node is None:
...
def delete_node(node, target):
    """Deletes 'target' from the BST rooted at 'node' and returns the
...
def print_in_order(node, output):
    """Collects the tree's values in ascending order (in-order traversal of
    a BST always yields sorted output)."""
    if node is None:
        return
    print_in_order(node.left, output)
    output.append(node.value)
...
if __name__ == "__main__":
    ...
    in_order_result = []
    print_in_order(root, in_order_result)
    print("In-order traversal:", " ".join(str(v) for v in in_order_result))

    print("search(40):", "found" if search(root, 40) else "not found")
    print("search(99):", "found" if search(root, 99) else "not found")

    # Delete a node with two children (30 has children 20 and 40)
    root = delete_node(root, 30)
    after_30 = []
    print_in_order(root, after_30)
    print("After deleting 30:", " ".join(str(v) for v in after_30))
...
`,
    ...
    // Returns true if 'target' exists anywhere in the BST rooted at 'node'.
    static boolean search(BSTNode node, int target) {
    ...
    // Finds the node holding the smallest value in a subtree, by always
    // following left-child pointers until none remain. Used during
    ...
    // Deletes 'target' from the BST rooted at 'node' and returns the
    // (possibly new) root of this subtree.
    static BSTNode deleteNode(BSTNode node, int target) {
    ...
    // Appends the tree's values in ascending order (in-order traversal of
    // a BST always yields sorted output).
    static void printInOrder(BSTNode node, StringBuilder output) {
        if (node == null) return;
        printInOrder(node.left, output);
        output.append(node.value).append(" ");
        printInOrder(node.right, output);
    ...
    public static void main(String[] args) {
        ...
        StringBuilder inOrderOutput = new StringBuilder();
        printInOrder(root, inOrderOutput);
        System.out.println("In-order traversal: " + inOrderOutput.toString().trim());

        System.out.println("search(40): " + (search(root, 40) ? "found" : "not found"));
        System.out.println("search(99): " + (search(root, 99) ? "found" : "not found"));

        // Delete a node with two children (30 has children 20 and 40)
        root = deleteNode(root, 30);
        StringBuilder after30 = new StringBuilder();
        printInOrder(root, after30);
        System.out.println("After deleting 30: " + after30.toString().trim());

        // Delete another node with two children (70 has children 60 and 80)
        root = deleteNode(root, 70);
        StringBuilder after70 = new StringBuilder();
        printInOrder(root, after70);
        System.out.println("After deleting 70: " + after70.toString().trim());
    }
}
`,
...
console.log("In-order traversal:", inOrderResult.join(" "));

console.log("search(40):", search(root, 40) ? "found" : "not found");
console.log("search(99):", search(root, 99) ? "found" : "not found");

// Delete a node with two children (30 has children 20 and 40)
root = deleteNode(root, 30);
const after30 = [];
collectInOrder(root, after30);
console.log("After deleting 30:", after30.join(" "));
...
// Returns 1 if 'target' exists anywhere in the BST rooted at 'node', else 0.
int search(BSTNode* node, int target) {
    if (node == NULL) return 0;
...
// Finds the node holding the smallest value in a subtree, by always
// following left-child pointers until none remain.
BSTNode* findMin(BSTNode* node) {
    while (node->left != NULL) {
        node = node->left;
    }
...
public static class BinarySearchTree {
    ...
    // Returns true if 'target' exists anywhere in the BST rooted at 'node'.
    static bool Search(BSTNode node, int target) {
        if (node == null) return false;
        if (node.Value == target) return true;
        if (target < node.Value) return Search(node.Left, target);
        return Search(node.Right, target);
    ...
    // Finds the node holding the smallest value in a subtree.
    static BSTNode FindMin(BSTNode node) {
        while (node.Left != null) {
            node = node.Left;
        }
    ...
    // Appends the tree's values in ascending order.
    static void PrintInOrder(BSTNode node, StringBuilder output) {
        if (node == null) return;
        PrintInOrder(node.Left, output);
        output.Append(node.Value).Append(" ");
    ...
    public static void Main() {
        ...
        var inOrderOutput = new StringBuilder();
        PrintInOrder(root, inOrderOutput);
        Console.WriteLine("In-order traversal: " + inOrderOutput.ToString().Trim());

        Console.WriteLine("search(40): " + (Search(root, 40) ? "found" : "not found"));
        Console.WriteLine("search(99): " + (Search(root, 99) ? "found" : "not found"));

        root = DeleteNode(root, 30);
        var after30 = new StringBuilder();
        PrintInOrder(root, after30);
        Console.WriteLine("After deleting 30: " + after30.ToString().Trim());

        root = DeleteNode(root, 70);
        var after70 = new StringBuilder();
        PrintInOrder(root, after70);
        Console.WriteLine("After deleting 70: " + after70.ToString().Trim());
    }
}
`,
        "swift": `import Foundation
...
// Inserts 'value' into the BST rooted at 'node' and returns the (possibly
// new) root of this subtree. Duplicate values are silently ignored.
func insert(_ node: BSTNode?, _ value: Int) -> BSTNode {
    guard let node = node else {
        return BSTNode(value)
    }
    if value < node.value {
...
// Returns true if 'target' exists anywhere in the BST rooted at 'node'.
func search(_ node: BSTNode?, _ target: Int) -> Bool {
...
// Finds the node holding the smallest value in a subtree.
func findMin(_ node: BSTNode) -> BSTNode {
    var current = node
...
// Deletes 'target' from the BST rooted at 'node' and returns the (possibly
// new) root of this subtree, or nil if the subtree becomes empty.
func deleteNode(_ node: BSTNode?, _ target: Int) -> BSTNode? {
...
func collectInOrder(_ node: BSTNode?, _ output: inout [Int]) {
    guard let node = node else { return }
    collectInOrder(node.left, &output)
    output.append(node.value)
    collectInOrder(node.right, &output)
...
let valuesToInsert = [50, 30, 70, 20, 40, 60, 80]
for value in valuesToInsert {
    root = insert(root, value)
}
...
var inOrderResult: [Int] = []
collectInOrder(root, &inOrderResult)
print("In-order traversal: \(inOrderResult.map { String($0) }.joined(separator: " "))")
...
var after30: [Int] = []
collectInOrder(root, &after30)
print("After deleting 30: \(after30.map { String($0) }.joined(separator: " "))")
...
// Returns true if 'target' exists anywhere in the BST rooted at 'node'.
fun search(node: BSTNode?, target: Int): Boolean {
    if (node == null) return false
    if (node.value == target) return true
    return if (target < node.value) search(node.left, target) else search(node.right, target)
}

// Finds the node holding the smallest value in a subtree.
fun findMin(node: BSTNode): BSTNode {
    var current = node
...
// Deletes 'target' from the BST rooted at 'node' and returns the (possibly
// new) root of this subtree, or null if the subtree becomes empty.
fun deleteNode(node: BSTNode?, target: Int): BSTNode? {
    if (node == null) return null
...
// Collects the tree's values in ascending order.
fun collectInOrder(node: BSTNode?, output: MutableList<Int>) {
    if (node == null) return
    collectInOrder(node.left, output)
    output.add(node.value)
    collectInOrder(node.right, output)
...
fun main() {
    ...
    val valuesToInsert = intArrayOf(50, 30, 70, 20, 40, 60, 80)
    for (value in valuesToInsert) {
        root = insert(root, value)
    }
    ...
    val inOrderResult = mutableListOf<Int>()
    collectInOrder(root, inOrderResult)
    println("In-order traversal: \${inOrderResult.joinToString(" ")}")

    println("search(40): \${if (search(root, 40)) "found" else "not found"}")
    println("search(99): \${if (search(root, 99)) "found" else "not found"}")

    root = deleteNode(root, 30)
    val after30 = mutableListOf<Int>()
    collectInOrder(root, after30)
    println("After deleting 30: \${after30.joinToString(" ")}")

    root = deleteNode(root, 70)
    val after70 = mutableListOf<Int>()
    collectInOrder(root, after70)
...
object BinarySearchTree extends App {
    ...
    // Inserts 'value' into the BST rooted at 'node' and returns the
    // (possibly new) root of this subtree. Duplicate values are ignored.
    def insert(node: BSTNode, value: Int): BSTNode = {
        if (node == null) {
            return new BSTNode(value)
        }
        if (value < node.value) {
    ...
    // Returns true if 'target' exists anywhere in the BST rooted at 'node'.
    def search(node: BSTNode, target: Int): Boolean = {
        if (node == null) return false
        if (node.value == target) return true
        if (target < node.value) search(node.left, target) else search(node.right, target)
    }
    ...
    // Deletes 'target' from the BST rooted at 'node' and returns the
    // (possibly new) root of this subtree, or null if the subtree becomes empty.
    def deleteNode(node: BSTNode, target: Int): BSTNode = {
        if (node == null) return null
    ...
    // Collects the tree's values in ascending order.
    def collectInOrder(node: BSTNode, output: scala.collection.mutable.ListBuffer[Int]): Unit = {
        if (node == null) return
        collectInOrder(node.left, output)
        output += node.value
        collectInOrder(node.right, output)
    ...
    val valuesToInsert = Array(50, 30, 70, 20, 40, 60, 80)
    for (value <- valuesToInsert) {
        root = insert(root, value)
    }

    val inOrderResult = scala.collection.mutable.ListBuffer[Int]()
    collectInOrder(root, inOrderResult)
    println(s"In-order traversal: \${inOrderResult.mkString(" ")}")

    println(s"search(40): \${if (search(root, 40)) "found" else "not found"}")
    println(s"search(99): \${if (search(root, 99)) "found" else "not found"}")

    root = deleteNode(root, 30)
    val after30 = scala.collection.mutable.ListBuffer[Int]()
    collectInOrder(root, after30)
    println(s"After deleting 30: \${after30.mkString(" ")}")

    root = deleteNode(root, 70)
    val after70 = scala.collection.mutable.ListBuffer[Int]()
    collectInOrder(root, after70)
...
// BSTNode is a standard BST node. Unlike some node-based structures
// elsewhere in this reference, a plain BST needs nothing beyond the key
...
// insert places 'value' into the BST rooted at 'node' and returns the
// (possibly new) root of this subtree. Duplicate values are ignored.
...
// search returns true if 'target' exists anywhere in the BST rooted at 'node'.
func search(node *BSTNode, target int) bool {
    if node == nil {
        return false
    }
    if node.value == target {
...
// findMin finds the node holding the smallest value in a subtree.
...
// deleteNode removes 'target' from the BST rooted at 'node' and returns
// the (possibly new) root of this subtree, or nil if the subtree becomes empty.
func deleteNode(node *BSTNode, target int) *BSTNode {
    if node == nil {
        return nil
    }
...
// collectInOrder appends the tree's values in ascending order.
func collectInOrder(node *BSTNode, output *[]int) {
    if node == nil {
        return
    }
    collectInOrder(node.left, output)
...
func main() {
    ...
    var after30 []int
    collectInOrder(root, &after30)
    fmt.Println("After deleting 30:", after30)
...
/// Inserts \`value\` into the BST rooted at \`node\` and returns the
/// (possibly new) root of this subtree. Duplicate values are ignored.
fn insert(node: Option<Box<BSTNode>>, value: i32) -> Option<Box<BSTNode>> {
    match node {
...
/// Removes and returns the minimum-value node from a subtree, rewiring the
/// subtree's remaining structure in place. Used during deletion of a node
/// with two children.
fn remove_min(node: Option<Box<BSTNode>>) -> (Option<Box<BSTNode>>, i32) {
    let mut node = node.unwrap();
    if node.left.is_none() {
        return (node.right.take(), node.value);
    }
    let (new_left, min_value) = remove_min(node.left.take());
    node.left = new_left;
    (Some(node), min_value)
}
...
/// Deletes \`target\` from the BST rooted at \`node\` and returns the
/// (possibly new) root of this subtree.
fn delete_node(node: Option<Box<BSTNode>>, target: i32) -> Option<Box<BSTNode>> {
    let mut node = match node {
        None => return None,
        Some(n) => n,
    };
    ...
    if target < node.value {
        node.left = delete_node(node.left.take(), target);
        Some(node)
    } else if target > node.value {
        node.right = delete_node(node.right.take(), target);
        Some(node)
    } else {
        // This is the node to delete.
        if node.left.is_none() {
        ...
        // Two children: pull the in-order successor (minimum of the right
        // subtree) out, use its value to replace this node's value, and
        // reattach the now-successor-free right subtree.
        let (new_right, successor_value) = remove_min(node.right.take());
...
/// Collects the tree's values in ascending order.
fn collect_in_order(node: &Option<Box<BSTNode>>, output: &mut Vec<i32>) {
    if let Some(n) = node {
        collect_in_order(&n.left, output);
        output.push(n.value);
        collect_in_order(&n.right, output);
...
fn main() {
    ...
    let values_to_insert = [50, 30, 70, 20, 40, 60, 80];
    for &value in values_to_insert.iter() {
        root = insert(root, value);
    }

    let mut in_order_result = Vec::new();
    collect_in_order(&root, &mut in_order_result);
    println!("In-order traversal: {:?}", in_order_result);

    println!("search(40): {}", search(&root, 40));
    println!("search(99): {}", search(&root, 99));

    root = delete_node(root, 30);
    let mut after_30 = Vec::new();
    collect_in_order(&root, &mut after_30);
    println!("After deleting 30: {:?}", after_30);

    root = delete_node(root, 70);
    let mut after_70 = Vec::new();
...
// Finds the Lowest Common Ancestor of the nodes holding values 'p' and 'q'
// within the tree rooted at 'node'. Assumes both p and q exist in the tree.
TreeNode* lowestCommonAncestor(TreeNode* node, int p, int q) {
    // Base case: an empty subtree, or we've directly found one of the targets.
    if (node == nullptr) return nullptr;
    ...
    // If both sides found something, this node is where the two paths meet.
    if (leftResult != nullptr && rightResult != nullptr) {
        return node;
    }

    // Otherwise, propagate whichever side found a result (or null if neither did).
...
int main() {
    ...
    TreeNode* ancestorOf5And1 = lowestCommonAncestor(root, 5, 1);
    cout << "LCA(5, 1) = " << (ancestorOf5And1 != nullptr ? ancestorOf5And1->value : -1) << endl;

    TreeNode* ancestorOf5And4 = lowestCommonAncestor(root, 5, 4);
    cout << "LCA(5, 4) = " << (ancestorOf5And4 != nullptr ? ancestorOf5And4->value : -1) << endl;
...
def lowest_common_ancestor(node, p, q):
    """Finds the Lowest Common Ancestor of the nodes holding values 'p' and
    'q' within the tree rooted at 'node'. Assumes both p and q exist in the
    tree."""
    # Base case: an empty subtree, or we've directly found one of the targets.
    if node is None:
        return None
    ...
    # Search both subtrees independently.
    left_result = lowest_common_ancestor(node.left, p, q)
    right_result = lowest_common_ancestor(node.right, p, q)

    # If both sides found something, this node is where the two paths meet.
    if left_result is not None and right_result is not None:
        return node

    # Otherwise, propagate whichever side found a result (or None if neither did).
...
if __name__ == "__main__":
    ...
    ancestor_of_5_and_1 = lowest_common_ancestor(root, 5, 1)
    print("LCA(5, 1) =", ancestor_of_5_and_1.value if ancestor_of_5_and_1 else -1)
...
`,
    ...
    // Finds the Lowest Common Ancestor of the nodes holding values 'p' and
    // 'q' within the tree rooted at 'node'. Assumes both p and q exist in
    // the tree.
    static TreeNode lowestCommonAncestor(TreeNode node, int p, int q) {
        // Base case: an empty subtree, or we've directly found one of the targets.
        if (node == null) return null;
        if (node.value == p || node.value == q) return node;
    ...
    public static void main(String[] args) {
        ...
        TreeNode ancestorOf5And1 = lowestCommonAncestor(root, 5, 1);
        System.out.println("LCA(5, 1) = " + (ancestorOf5And1 != null ? ancestorOf5And1.value : -1));

        TreeNode ancestorOf5And4 = lowestCommonAncestor(root, 5, 4);
        System.out.println("LCA(5, 4) = " + (ancestorOf5And4 != null ? ancestorOf5And4.value : -1));
    }
}
`,
...
function lowestCommonAncestor(node, p, q) {
    ...
    // Search both subtrees independently.
    const leftResult = lowestCommonAncestor(node.left, p, q);
    const rightResult = lowestCommonAncestor(node.right, p, q);
...
const ancestorOf5And1 = lowestCommonAncestor(root, 5, 1);
console.log("LCA(5, 1) =", ancestorOf5And1 !== null ? ancestorOf5And1.value : -1);
...
int main(void) {
    ...
    TreeNode* ancestorOf5And1 = lowestCommonAncestor(root, 5, 1);
    printf("LCA(5, 1) = %d\n", ancestorOf5And1 != NULL ? ancestorOf5And1->value : -1);

    TreeNode* ancestorOf5And4 = lowestCommonAncestor(root, 5, 4);
    printf("LCA(5, 4) = %d\n", ancestorOf5And4 != NULL ? ancestorOf5And4->value : -1);
...
public static class LowestCommonAncestor {
    ...
    // 'q' within the tree rooted at 'node'. Assumes both p and q exist in
    // the tree.
    static TreeNode FindLowestCommonAncestor(TreeNode node, int p, int q) {
        if (node == null) return null;
        if (node.Value == p || node.Value == q) return node;

        TreeNode leftResult = FindLowestCommonAncestor(node.Left, p, q);
        TreeNode rightResult = FindLowestCommonAncestor(node.Right, p, q);
    ...
    public static void Main() {
        ...
        TreeNode ancestorOf5And1 = FindLowestCommonAncestor(root, 5, 1);
        Console.WriteLine("LCA(5, 1) = " + (ancestorOf5And1 != null ? ancestorOf5And1.Value.ToString() : "-1"));

        TreeNode ancestorOf5And4 = FindLowestCommonAncestor(root, 5, 4);
        Console.WriteLine("LCA(5, 4) = " + (ancestorOf5And4 != null ? ancestorOf5And4.Value.ToString() : "-1"));
    }
}
`,
        "swift": `import Foundation
...
// Finds the Lowest Common Ancestor of the nodes holding values 'p' and 'q'
// within the tree rooted at 'node'. Assumes both p and q exist in the tree.
func lowestCommonAncestor(_ node: TreeNode?, _ p: Int, _ q: Int) -> TreeNode? {
    guard let node = node else { return nil }
    if node.value == p || node.value == q { return node }
...
let ancestorOf5And1 = lowestCommonAncestor(root, 5, 1)
print("LCA(5, 1) = \(ancestorOf5And1?.value ?? -1)")
...
// Finds the Lowest Common Ancestor of the nodes holding values 'p' and 'q'
// within the tree rooted at 'node'. Assumes both p and q exist in the tree.
fun lowestCommonAncestor(node: TreeNode?, p: Int, q: Int): TreeNode? {
    if (node == null) return null
    if (node.value == p || node.value == q) return node

    val leftResult = lowestCommonAncestor(node.left, p, q)
    val rightResult = lowestCommonAncestor(node.right, p, q)
...
fun main() {
    ...
    val ancestorOf5And1 = lowestCommonAncestor(root, 5, 1)
    println("LCA(5, 1) = \${ancestorOf5And1?.value ?: -1}")

    val ancestorOf5And4 = lowestCommonAncestor(root, 5, 4)
...
object LowestCommonAncestor extends App {
    ...
    // 'q' within the tree rooted at 'node'. Assumes both p and q exist in
    // the tree.
    def lowestCommonAncestor(node: TreeNode, p: Int, q: Int): TreeNode = {
        if (node == null) return null
        if (node.value == p || node.value == q) return node
        ...
        if (leftResult != null) leftResult else rightResult
    }
    ...
    val ancestorOf5And1 = lowestCommonAncestor(root, 5, 1)
    println(s"LCA(5, 1) = \${if (ancestorOf5And1 != null) ancestorOf5And1.value else -1}")
...
// TreeNode is a minimal binary tree node used to demonstrate the LCA
// algorithm.
type TreeNode struct {
    value int
    left  *TreeNode
...
// lowestCommonAncestor finds the Lowest Common Ancestor of the nodes
// holding values 'p' and 'q' within the tree rooted at 'node'. Assumes
// both p and q exist in the tree.
func lowestCommonAncestor(node *TreeNode, p int, q int) *TreeNode {
    if node == nil {
        return nil
    }
...
func main() {
    ...
    ancestorOf5And1 := lowestCommonAncestor(root, 5, 1)
    if ancestorOf5And1 != nil {
        fmt.Println("LCA(5, 1) =", ancestorOf5And1.value)
    } else {
        fmt.Println("LCA(5, 1) = -1")
    }

    ancestorOf5And4 := lowestCommonAncestor(root, 5, 4)
    if ancestorOf5And4 != nil {
        fmt.Println("LCA(5, 4) =", ancestorOf5And4.value)
    } else {
        fmt.Println("LCA(5, 4) = -1")
    }
}
`,
        "rust": `use std::rc::Rc;
...
/// A minimal binary tree node used to demonstrate the LCA algorithm.
/// Rc<RefCell<..>> allows the tree to be shared and traversed safely.
struct TreeNode {
    value: i32,
    left: Option<Rc<RefCell<TreeNode>>>,
    right: Option<Rc<RefCell<TreeNode>>>,
...
fn lowest_common_ancestor(
    ...
    let left_result = lowest_common_ancestor(&current.borrow().left, p, q);
    let right_result = lowest_common_ancestor(&current.borrow().right, p, q);
...
fn main() {
    ...
    let ancestor_of_5_and_1 = lowest_common_ancestor(&root_option, 5, 1);
    match ancestor_of_5_and_1 {
        Some(node) => println!("LCA(5, 1) = {}", node.borrow().value),
        None => println!("LCA(5, 1) = -1"),
    }

    let ancestor_of_5_and_4 = lowest_common_ancestor(&root_option, 5, 4);
    match ancestor_of_5_and_4 {
        Some(node) => println!("LCA(5, 4) = {}", node.borrow().value),
