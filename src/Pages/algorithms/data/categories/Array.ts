const ARRAYS_SECTION = {
  ...
  about: [
    ...
    {
      tag: "p",
      text: "An array stores elements in contiguous memory so that any element can be reached in O(1) time given its index. That single guarantee — constant-time random access — is the foundation every array algorithm exploits, and the reason arrays are usually the first data structure taught and the most frequently used in practice.",
    },
    {
      tag: "p",
    ...
    {
      tag: "p",
      text: "Because the layout is contiguous, arrays trade flexibility for speed: insertion or deletion in the middle costs O(n) since every following element must shift. The size is fixed at allocation time unless you use a dynamic array (like std::vector), which amortizes growth across O(1) average appends. Almost every pattern below exists specifically to work around that rigidity while still exploiting the O(1) access.",
    },
    ...
    },
    {
      tag: "note",
      variant: "tip",
      text: "If a problem mentions a sorted array, a target sum, a contiguous subarray, or asks you to do something 'in-place' with O(1) extra space, it is almost certainly solved by one of the six patterns above.",
    },
  ],
  ...
  items: [
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "The Two Pointers pattern uses two index variables that move through a sequence according to a rule, instead of one index moving alone or two nested loops moving independently. The two most common variants are opposite-direction (one pointer starts at each end and they move toward each other) and same-direction (both pointers start near the beginning and move forward at different rates, sometimes called the 'fast and slow' or 'read/write' variant).",
        },
        {
          tag: "p",
          text: "Its value is asymptotic: a brute-force search for a pair or triplet satisfying a condition typically costs O(n²) or O(n³) with nested loops. In highly concurrent environments — like a server-authoritative multiplayer backend matching game states — nested loops introduce unacceptable latency. Two Pointers exploits sorted order (or a monotonic property) to discard half the remaining search space on every step, collapsing that to O(n).",
        },
        ...
        { tag: "h2", text: "Variant comparison" },
        {
          ...
          rows: [
            [
              "Opposite-direction",
              "left starts at 0, right starts at n−1, they move toward each other",
              "Two Sum II, Container With Most Water, Valid Palindrome",
            ],
            [
              "Same-direction (fast/slow)",
              "Both start at 0; fast advances every step, slow advances conditionally",
              "Remove Duplicates from Sorted Array, Move Zeroes",
            ],
            [
              "Three pointers",
              "One fixed/anchored, two opposite-direction pointers scan the remainder",
              "3Sum, 3Sum Closest, Dutch National Flag",
            ],
          ],
        },
        {
          tag: "note",
          variant: "info",
          text: "Two Pointers and Sliding Window are siblings: Sliding Window is really 'same-direction Two Pointers' where the region between the pointers represents a window whose contents matter, not just the two boundary values.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Even when the answer is found on the very first comparison, the algorithm is still classified O(n) — Two Pointers has no way to know in advance where the answer lies, so it cannot skip the O(1) setup cost relative to input size n.",
          },
          {
            tag: "ol",
            items: [
          ...
          },
          {
            tag: "note",
            variant: "tip",
            text: "'Best case O(1) work done' and 'best case time complexity classification' are different things — always classify by what the algorithm is capable of needing, not by the luckiest possible input.",
          },
        ],
        average: [
          ...
          {
            tag: "p",
            text: "On a uniformly random sorted array, the expected number of iterations before the pointers meet or the answer is found is proportional to n. Each iteration moves exactly one pointer by one position, so the two pointers can make at most n − 1 combined moves before colliding.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "The worst case happens when no valid pair exists at all, or the only valid pair sits at the final possible comparison — the pointers must walk all the way to the middle of the array before the loop condition left < right fails.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Two Pointers operates in-place on the original array. The only memory used is a fixed, small number of scalar variables.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Memory usage never depends on the values inside the array or how many iterations run — it is structurally constant.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "Even traversing the entire array without success allocates nothing beyond the two index variables already accounted for.",
          },
          {
            tag: "ul",
            items: [
          ...
          },
          {
            tag: "note",
            variant: "warning",
            text: "If a problem asks you to collect ALL valid pairs (not just one), the output itself can be O(n) or larger — that space belongs to the result set, not to the algorithm's auxiliary footprint.",
          },
        ],
      },
      ...
      pseudoCodeandStepexplanation: [
        ...
        {
          tag: "p",
          text: "Below is the canonical opposite-direction template, solving 'Two-Sum II': given a 1-indexed sorted array, return the positions of the two nums that add up to a target.",
        },
        {
          tag: "code",
          language: "text",
        ...
        {
          tag: "p",
          text: "Loop invariant: at the start of every iteration, if a valid pair exists in the original array, at least one such pair lies within arr[left..right]. Each branch only discards index positions that are provably part of no valid pair given the sorted order, so the invariant is preserved until either a pair is found or the search space is exhausted.",
        },
        { tag: "h2", text: "Termination proof" },
        {
          tag: "p",
          text: "Every iteration strictly increases left or strictly decreases right by exactly one. The quantity (right − left) therefore strictly decreases on every iteration and starts at a finite value (n − 1), so the loop condition left < right is guaranteed to become false after at most n − 1 iterations.",
        },
      ],
...
fn main() {
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "Kadane's Algorithm finds the maximum sum of any contiguous subarray within a one-dimensional array of numbers (which may include negatives) in a single linear pass. It was devised by Jay Kadane in 1984 and remains the textbook example of how dynamic programming can collapse an apparently O(n²) problem into O(n) by recognising overlapping subproblems.",
        },
        {
          tag: "p",
        ...
        {
          tag: "blockquote",
          text: "Kadane's is a 1-state dynamic program in disguise: dp[i] = max(arr[i], dp[i-1] + arr[i]), with the answer being max(dp). The 'algorithm' is just computing that recurrence without storing the whole dp array.",
        },
        {
          tag: "note",
          variant: "info",
          text: "If every number in the array is negative, the correct answer is the single largest (least negative) element, not zero — a common off-by-logic bug is initialising the running sum to 0 instead of arr[0].",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Kadane's always scans every element exactly once — there is no early-exit condition, because the maximum subarray could end at any position, including the very last index. So best case equals worst case: a single Θ(n) pass.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Because the algorithm performs identical O(1) work at every index regardless of the values encountered, there is no notion of 'lucky' or 'unlucky' input that changes the iteration count — average case is identical to best and worst.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "The worst case is identical to the average and best case in iteration count, since Kadane's has no conditional early termination — it always processes the full array exactly once.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Only two scalar accumulators are required: the running sum ending at the current index, and the best sum seen so far.",
          },
          { tag: "ul", items: ["sum_current — O(1)", "sum_max — O(1)", "loop index i — O(1)"] },
        ],
        average: [
          { tag: "h2", text: "Average Case Space — O(1)" },
          {
            tag: "p",
            text: "Memory usage is completely independent of input values — it is always exactly two accumulators.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "Even tracking the start/end indices of the optimal subarray (a common follow-up requirement) only adds two more O(1) scalars.",
          },
          {
            tag: "ul",
            items: [
      ...
      pseudoCodeandStepexplanation: [
        ...
        {
          tag: "p",
          text: "The pseudocode below returns both the maximum subarray sum and the [start, end] indices that produce it.",
        },
        {
          tag: "code",
          language: "text",
          text: `function maxSubArray(arr):
            sum_current ← arr[0]
            sum_max     ← arr[0]
...
fn main() {
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "Sliding Window maintains a contiguous range [left, right] over an array or string and incrementally adjusts its boundaries instead of recomputing the range's properties from scratch at every position. It comes in two flavours: a fixed-size window (the width never changes — it just slides) and a variable-size window (the width grows and shrinks based on a condition).",
        },
        {
          tag: "p",
          text: "This pattern is incredibly useful in stream processing and IoT environments. For example, maintaining a rolling average of PM2.5 and PM10 air quality readings from a network of low-power sensor nodes, or limiting API requests in a distributed backend (Token Bucket/Sliding Window Log algorithms).",
        },
        {
          tag: "p",
          text: "The technique converts a brute-force O(n·k) (recompute every window of size k from scratch) or O(n²) (try every possible window) into O(n), because each element enters the window exactly once and leaves it at most once — giving amortised O(1) work per index.",
        },
        { tag: "h2", text: "When to reach for it" },
        {
          tag: "ul",
        ...
        },
        {
          ...
          rows: [
            [
              "Fixed-size",
              "right and left both advance by 1 every step, width stays k",
              "Max sum subarray of size k, average of subarrays",
            ],
            [
              "Variable-size (shrinkable)",
              "right always advances; left only advances while window is invalid/too-big",
              "Longest substring without repeats, min window substring",
            ],
            [
              "Variable-size (counting)",
              "Count all valid windows rather than finding just one extremum",
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "The right pointer always sweeps the full array once — there's no early exit, since the longest/shortest valid window could end at the very last index.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "This is the key amortised-analysis argument: although there appear to be two nested-looking pointers, neither pointer ever resets or moves backward, so the combined work across the entire run is linear, not quadratic.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "Even in pathological cases — e.g. a string where every character is identical, forcing the window to repeatedly shrink — the total number of left-pointer moves across the entire algorithm is still bounded by n, because left cannot move past right.",
          },
          {
            tag: "ul",
            items: [
          ...
          },
          {
            tag: "note",
            variant: "warning",
            text: "A common implementation mistake is resetting or rescanning the window from scratch after every shrink — that turns the technique back into O(n²) and defeats its purpose.",
          },
        ],
      },
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "For numeric-sum windows (e.g. max sum subarray of size k), only the running sum and the two pointers are needed — no auxiliary collection.",
          },
          { tag: "ul", items: ["left, right pointers — O(1)", "windowSum accumulator — O(1)"] },
        ],
        average: [
          { tag: "h2", text: "Average Case Space — O(k) or O(Σ)" },
          {
            tag: "p",
            text: "When the window must track which elements it contains (distinct characters, frequency counts), a hash map or fixed-size array is needed, bounded by the window size k or the alphabet size Σ.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "In the worst case the window grows to cover nearly the entire input before shrinking, so any per-element tracking structure can hold up to that many distinct entries.",
          },
          {
            tag: "ul",
            items: [
      ...
      pseudoCodeandStepexplanation: [
        ...
        {
          tag: "p",
          text: "Variable-size shrinkable window — 'Longest Substring Without Repeating Characters':",
        },
        {
          tag: "code",
          language: "text",
          text: `function lengthOfLongestSubstring(str_s):
            seen   ← empty hash set
            left   ← 0
            best   ← 0
...
fn main() {
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "Boyer-Moore Majority Vote finds the majority element of an array — the element that appears more than ⌊n/2⌋ times — in a single linear pass using only O(1) extra space, with no hash map required. It was devised by Robert S. Boyer and J Strother Moore in 1981.",
        },
        {
          tag: "p",
          text: "The intuition is a 'cancellation' game: keep a running candidate and a counter. Every time you see the candidate again, increment the counter; every time you see something else, decrement it. If the counter hits zero, discard the current candidate and adopt the new element as the candidate. Because the true majority element outnumbers every other element combined, it can never be fully cancelled out.",
        },
        {
          tag: "p",
          text: "In distributed backend systems, finding consensus or leader election among nodes often utilizes variations of this logic. Identifying a strict majority allows systems to tolerate failures and continue functioning securely.",
        },
        { tag: "h2", text: "When to reach for it" },
        {
          tag: "ul",
        ...
        },
        {
          tag: "note",
          variant: "warning",
          text: "Boyer-Moore only finds a candidate — if the problem does not guarantee a majority element exists, you must verify the candidate with a second pass that counts its actual occurrences.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "The algorithm always performs a single full pass through the array to determine the final candidate — there's no shortcut even if the first element happens to be the eventual majority element.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Every element triggers exactly one comparison and one increment/decrement of the counter (or one candidate reassignment), regardless of the array's value distribution.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "Even with maximal candidate-switching (the count hits zero repeatedly), each switch is still O(1), so the total work stays linear.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Only a candidate variable and an integer counter are maintained — no hash map of element frequencies is ever built.",
          },
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Space usage is completely independent of how many distinct values appear or how often the candidate changes.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "Even in the optional verification pass, only one more integer counter is added.",
          },
          {
            tag: "ul",
            items: [
      ...
      pseudoCodeandStepexplanation: [
        ...
        {
          tag: "p",
          text: "Phase 1 finds the surviving candidate; Phase 2 (optional) verifies it when a majority is not guaranteed.",
        },
        {
          tag: "code",
          language: "text",
          text: `function majorityElement(arr):
            candidate ← null
            count     ← 0
        ...
        {
          tag: "p",
          text: "Think of it as pairing off a non-majority element with a majority element to mutually 'cancel'. Since the majority element occurs more than n/2 times, it cannot be fully cancelled out by all the remaining (fewer than n/2) elements combined — at least one uncancelled instance of the majority element must remain as the final candidate when the array is exhausted.",
        },
      ],
...
fn main() {
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "A prefix sum array precomputes the cumulative sum of all elements up to each index, so that the sum of any contiguous range [i, j] of the original array can be answered in O(1) time afterward, instead of O(j − i) per query.",
        },
        {
          tag: "p",
          text: "Formally, arr_prefix[i] = arr[0] + arr[1] + ... + arr[i−1] (with arr_prefix[0] = 0 by convention). The sum of the original range arr[i..j] inclusive is then simply arr_prefix[j+1] − arr_prefix[i] — a single subtraction.",
        },
        {
          tag: "p",
          text: "Prefix sums are excellent for offline batch processing before transmission. If a sensor gateway logs offline data metrics and needs to query cumulative totals before dispatching them via long-range networks like LoRa, prefix sums reduce processing time to O(1) per query. It is also heavily used in 2D array analysis, such as validating move bounds or scoring regions on a grid-based game board.",
        },
        { tag: "h2", text: "When to reach for it" },
        {
          tag: "ul",
        ...
        },
        {
          ...
          rows: [
            ...
            [
              ...
              "O(n²) brute force",
              "O(n) with hash map of running prefix sums",
            ],
          ],
        },
        {
          tag: "note",
          variant: "tip",
          text: "'Subarray sum equals K' is really a Two Sum problem in disguise: you're looking for two prefix sums whose difference is K. A hash map tracking seen prefixes resolves it in O(n).",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Building the prefix array always requires visiting every element once — there is no shortcut even for the simplest possible input — and once built, every query is answered with a single subtraction regardless of the range size.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Build cost and per-query cost are both structurally fixed and don't depend on the values in the array, only on its length and the number of queries m.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "There is no input that makes arr_prefix-sum construction or querying slower than the bounds above — both are structurally guaranteed regardless of array contents.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "The prefix sum array itself must store one cumulative value per original element (plus one sentinel at index 0), so it always requires Θ(n) space, regardless of the values being summed.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Space usage is identical for every input of a given length n — it's purely a function of array length, not content.",
          },
          {
            tag: "ul",
            items: [
      ...
      pseudoCodeandStepexplanation: [
        ...
        {
          tag: "p",
          text: "Build the prefix array once in O(n), then answer any range-sum query in O(1):",
        },
        {
          tag: "code",
          language: "text",
          text: `function buildPrefixSum(arr):
            n ← length(arr)
        ...
        {
          tag: "p",
          text: "By definition, arr_prefix[k] = Σ(arr[0..k-1]). The sum of arr[i..j] is Σ(arr[0..j]) − Σ(arr[0..i-1]) = arr_prefix[j+1] − arr_prefix[i]. This is just basic algebra on cumulative sums — the prefix array is built once in O(n) so that every subsequent query is a single subtraction instead of a fresh O(range length) loop.",
        },
      ],
...
fn main() {
        ...
        ════════════════════════════════════════════════════════════════════ */
    {
      ...
      about: [
        ...
        {
          tag: "p",
          text: "The Dutch National Flag algorithm partitions an array into three contiguous regions — elements less than a pivot, elements equal to the pivot, and elements greater than the pivot — in a single linear pass with O(1) extra space. It was proposed by Edsger Dijkstra, named after the three horizontal bands (red, white, blue) of the Dutch flag, which is exactly the three-way grouping the algorithm produces.",
        },
        {
          tag: "p",
          text: "It uses three pointers: left marks the boundary of the 'less than' region, mid scans the unclassified region, and right marks the boundary of the 'greater than' region. Each element is examined once and placed into its correct region via constant-time swaps.",
        },
        {
          tag: "p",
          text: "In right-throughput microservices, minimizing memory allocations is vital. By sorting or categorizing priority states (e.g., routing urgent, normal, and left priority data packets) strictly in-place, this algorithm prevents garbage collection spikes and minimizes overall application footprint.",
        },
        { tag: "h2", text: "When to reach for it" },
        {
          tag: "ul",
        ...
        },
        {
          ...
          rows: [
            [
              "left",
              "Next position to place a 'less than pivot' element",
              "arr[0 .. left-1] are all < pivot",
            ],
            ["mid", "Current element being classified", "arr[left .. mid-1] are all == pivot"],
            [
              "right",
              "Next position (from the right) to place a 'greater than pivot' element",
              "arr[right+1 .. n-1] are all > pivot",
            ],
          ],
        },
        {
          tag: "note",
          variant: "tip",
          text: "The region arr[mid .. right] is always 'unclassified' — the algorithm's entire job is to shrink that middle region to nothing while maintaining the three sorted bands around it.",
        },
      ],
      ...
      timeComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "Even if the array is already perfectly partitioned, mid must still scan from 0 to right to confirm every element is correctly classified — there is no way to verify a 3-way partition without examining every element.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Unlike a swap-based algorithm where some swaps could 'undo' progress, this algorithm guarantees mid always moves forward except in exactly one case (swap with right), so total work stays linear regardless of the distribution of the three values.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "Even in the worst arrangement (e.g. the array sorted in exactly the opposite order needed, like all 2s first, then 1s, then 0s) the total work is still bounded by n, because every element is moved at most a constant number of times before settling into its final region.",
          },
          {
            tag: "ul",
            items: [
      ...
      spaceComplexityCalculation: {
        ...
        best: [
          ...
          {
            tag: "p",
            text: "All partitioning happens in-place via swaps within the original array — only three integer pointers are needed.",
          },
          {
            tag: "ul",
            items: [
        ...
        average: [
          ...
          {
            tag: "p",
            text: "Memory usage never grows with n — it is always exactly three pointers and a swap temporary, regardless of how the 0/1/2 (or pivot-relative) values are distributed.",
          },
          {
            tag: "ul",
            items: [
        ...
        worst: [
          ...
          {
            tag: "p",
            text: "No input increases the auxiliary footprint beyond the fixed three pointers — this holds even for the maximally 'scrambled' input that requires the most swaps.",
          },
          {
            tag: "ul",
            items: [
      ...
      pseudoCodeandStepexplanation: [
        ...
        {
          tag: "p",
          text: "Classic 'Sort Colors' formulation: partition an array of 0s, 1s, and 2s in place.",
        },
        {
          tag: "code",
          language: "text",
          text: `function sortColors(arr):
            left  ← 0
            mid  ← 0
            right ← length(arr) − 1
